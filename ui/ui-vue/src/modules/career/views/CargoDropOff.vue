    ...
    -->
      ...
      <BngCard v-if="mode === MODES.cargoSelection">
        ...
        <div class="card-content">
          <BngPropVal
            v-if="data.customAmountPerMaterialType.some(info => !info.virtualCapacity)"
            class="limited-capacity-info"
            :valueLabel="'This facility has limited capacity for ordinary cargo.'"
            :iconType="icons.info"/>
          <div class="scroll-wrapper">
            <template v-for="info in data.customAmountPerMaterialType">
              <CardGroup class="fullwidth-group" :label="info.material.name" :meta="info.meta">
                <BngPropVal
                  v-if="info.virtualCapacity"
                  valueLabel="Project capacity is reserved for the full contract load."
                  :iconType="icons.checkmark"/>
        ...
        <div class="card-content">
          ...
          <div v-if="summary.detailledList.length <= 1 || cargoOverviewStore.detailedDropOff" class="rewards-breakdown-container padding-bottom">
            ...
            <div class="grid-wrapper">
              ...
              <BngDivider />
              <DemandBonusRow :demand-info="demandInfo" />
...
import DemandBonusRow from "./DemandBonusRow.vue"
...
const data = ref({})
const summary = ref([])
const demandInfo = computed(() => data.value && data.value.demandInfo ? data.value.demandInfo : null)
...
data.value.customAmountPerMaterialType.forEach(info => {
  info.items.forEach(item => {
    const amount = Number(item.amountSelector?.value ?? item.amountSelector) || 0
    if (amount > 0) {
      confirmedCargoIds.push({
        id: item.ids[0],
        ids: item.ids,
        amount: amount,
      })
...
let newRewards = []
const metaKeys = { economyMultiplier: true, moneyMultiplier: true }

for (let key in rewards) {
  if (metaKeys[key]) continue
  let amount = rewards[key]
  if (typeof amount !== "number") continue
  if (!branchInfo || !branchInfo[key]) continue
  let rewardInfo = {
    attributeKey: key,
    rewardAmount: amount,
