<template>
  <div class="techs-tab" :class="{ 'techs-tab--phone': layout === 'phone' }">
    <div class="tab-header">
      <h2>Drivers</h2>
      <p v-if="store.businessType === 'racingTeam'">
        Manage your racing roster and assign fleet cars to drivers.
      </p>
      <p v-else>Manage your roster and assign goals from the desk.</p>
    </div>
    <div class="techs-info-banners">
      <div class="techs-summary-banner" v-if="techCapabilityTier || hasManager">
        ...
        <div class="summary-section" v-if="hasManager">
          ...
          <button
            type="button"
            class="btn btn-secondary manager-pause-btn"
    ...
    <div v-if="techList.length" class="techs-grid">
      ...
      >
        <div class="tech-card__header">
          ...
          <div class="tech-card__success-rate" v-if="store.businessType !== 'racingTeam' && tech.successRate !== undefined && !tech.fired">
            <span class="success-rate-value">{{ tech.successRate }}%</span>
          </div>
          <div class="tech-card__racing-badge" v-else-if="store.businessType === 'racingTeam' && !tech.fired && tech.driverSkillClass">
            <span class="racing-class-value">{{ tech.driverSkillClass }}</span>
          </div>
        ...
        <div class="tech-card__body">
          <div class="tech-card__row" v-if="!tech.fired && store.businessType === 'racingTeam'">
            <span class="label">Fleet car</span>
            <span class="value" :class="{ highlight: tech.fleetVehicleName }">
              {{ tech.fleetVehicleName || "Not assigned" }}
            </span>
          </div>
          <div class="tech-card__row" v-if="!tech.fired">
            ...
            <span class="value">
              ...
              <template v-else-if="store.businessType === 'racingTeam' && remainingSecondsForPostRaceCooldown(tech) > 0">
          ...
          <div class="tech-card__stats-grid" v-if="!tech.fired">
            <template v-if="store.businessType === 'racingTeam'">
              <div class="stat-item">
                <span class="stat-label">Racing XP</span>
                <span class="stat-value">{{ tech.racingSkillXp ?? 0 }}</span>
              </div>
              <div class="stat-item">
                <span class="stat-label">Races</span>
                <span class="stat-value">{{ tech.sanctionedRacesFinished ?? 0 }}</span>
              </div>
              <div class="stat-item">
                <span class="stat-label">Wins</span>
                <span class="stat-value">{{ tech.sanctionedRaceWins ?? 0 }}</span>
              </div>
              <div class="stat-item">
                <span class="stat-label">Podiums</span>
                <span class="stat-value">{{ tech.sanctionedPodiums ?? 0 }}</span>
              </div>
            </template>
          ...
          <div v-if="tech.fired" class="assign-panel">
            <button
              type="button"
              class="btn btn-primary assign-panel__btn"
          ...
          <div v-else-if="!tech.jobId" class="assign-panel">
            <button
              ...
              type="button"
              class="btn btn-primary assign-panel__btn"
              :disabled="fleetVehicles.length === 0"
            ...
            >
              <span class="btn-icon">+</span>
              {{ availableJobs.length === 0 ? "No Goals Available" : "Assign New Goal" }}
            </button>
            <button
              type="button"
              class="btn btn-secondary assign-panel__btn"
    ...
    <div v-if="techList.length === 0" class="empty-state">
      ...
      <h3>No Drivers Hired</h3>
      <p v-if="store.businessType === 'racingTeam' && store.currentLeague === 'league1'">Complete league 1 to unlock drivers</p>
      <p v-else>Hire drivers through upgrades and skill trees as you expand the team.</p>
    </div>
        ...
        >
          ...
          >
            <div class="job-select-modal__header">
              <h3>{{ store.businessType === 'racingTeam' ? 'Assign fleet car' : 'Assign goal to driver' }}</h3>
              <button
            ...
            <div class="job-select-modal__content">
              <template v-if="store.businessType === 'racingTeam'">
                <div v-if="fleetVehicles.length === 0" class="job-select-modal__empty">
                  <p>No vehicles in your fleet.</p>
                  <small>Purchase or store vehicles for this team first.</small>
                </div>
                <div v-else class="job-select-modal__grid">
                  <FleetAssignCard
                    ...
                    :vehicle="v"
                    @assign="selectFleetVehicle"
                  />
                </div>
              </template>
              <template v-else>
                <div v-if="availableJobs.length === 0" class="job-select-modal__empty">
                  <p>No available active goals.</p>
                  <small>Accept goals from the Goals tab first.</small>
                </div>
                <div v-else class="job-select-modal__grid">
                  <JobAssignCard
                    ...
                    :job="job"
                    @assign="selectJob(techByOpenModalId, $event.jobId ?? $event.id)"
                  />
                </div>
              </template>
            </div>
...
<script setup>
import { computed, ref, Teleport, nextTick, onMounted, onUnmounted, watch, inject } from "vue"
...
import FleetAssignCard from "./FleetAssignCard.vue"
...
const driversUiTick = ref(0)
let driversUiSecondTimer = null

/** Same pattern as scheduled sanctioned races: wall epoch ticks every real second in the UI. */
const remainingSecondsForPostRaceCooldown = (tech) => {
  const wallDue = Number(tech?.postRaceCooldownReadyWallEpoch)
  void driversUiTick.value
...
/** Counter style: M:SS, or H:MM:SS when over an hour. */
...
const managerCantAssignMessage = computed(() => {
  ...
  return "Manager can't assign: no free active goal slots"
})
...
const techByOpenModalId = computed(() => {
  const id = openModalTechId.value
  if (id === null || id === undefined) return undefined
  return techList.value.find(t => String(t.id) === String(id))
})
...
const selectFleetVehicle = async (vehicle) => {
  const techId = openModalTechId.value
  if (techId === null || techId === undefined) return
  const vid = vehicle?.vehicleId ?? vehicle?.id
  if (vid === undefined || vid === null || vid === "") return
  const success = await store.assignFleetVehicleToDriver(techId, vid)
  if (success) {
    closeJobModal()
  }
}
...
const handleClickOutside = (event) => {
  if (openModalTechId.value && !event.target.closest('.job-select-modal') && !event.target.closest('.assign-panel__btn')) {
    closeJobModal()
  }
}
...
onMounted(() => {
  ...
  driversUiSecondTimer = window.setInterval(() => {
    driversUiTick.value++
  }, 1000)
...
onUnmounted(() => {
  ...
  if (driversUiSecondTimer != null) {
    clearInterval(driversUiSecondTimer)
    driversUiSecondTimer = null
  }
...
.techs-tab--phone {
  ...
  .tech-card__racing-badge .racing-class-value {
    font-size: 0.8em;
  }
