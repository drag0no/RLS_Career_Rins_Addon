<template>
  <div class="finances-tab">
    ...
    <div v-if="financesData && !loading" class="finances-content">
      ...
      <div class="finances-section sale-section">
        <div class="sale-header">
          <div>
            <h3>Sell Business</h3>
            <p>Liquidate this business and close its account.</p>
          </div>
          <button
            type="button"
            ...
            data-focusable
          >
            {{ saleLoading ? 'Quoting...' : 'Sell Business' }}
          </button>
        </div>
      </div>
    </div>
    ...
    <Teleport to="body">
      <transition name="modal-fade">
        <div v-if="showSaleModal" class="sale-modal-overlay" @click.self="closeSaleModal">
          <div class="sale-modal" ref="saleModalRef">
            <h2>Sell {{ saleQuote?.businessName || 'Business' }}</h2>
            <p class="sale-warning">
              Vehicles, parts, jobs, staff, kits, offers, and business progress will be liquidated or reset.
            </p>

            <div class="sale-summary" v-if="saleQuote">
              <div class="sale-row">
                <span>Base resale</span>
                <strong>{{ formatCurrency(saleQuote.baseResaleValue || 0) }}</strong>
              </div>
              <div class="sale-row">
                <span>Asset liquidation</span>
                <strong>{{ formatCurrency(saleQuote.assetLiquidationValue || 0) }}</strong>
              </div>
              <div class="sale-row">
                <span>Upgrade liquidation</span>
                <strong>{{ formatCurrency(saleQuote.upgradeLiquidationValue || 0) }}</strong>
              </div>
              <div class="sale-row">
                <span>Account balance</span>
                <strong :class="{ negative: (saleQuote.accountBalance || 0) < 0 }">{{ formatCurrency(saleQuote.accountBalance || 0) }}</strong>
              </div>
              <div class="sale-row">
                <span>Loan payoff</span>
                <strong class="negative">-{{ formatCurrency(saleQuote.loanPayoffAmount || 0) }}</strong>
              </div>
              <div class="sale-row">
                <span>Debt converted</span>
                <strong :class="{ negative: (saleQuote.convertedDebt || 0) > 0 }">{{ formatCurrency(saleQuote.convertedDebt || 0) }}</strong>
              </div>
              <div class="sale-row total">
                <span>Net payout</span>
                <strong>{{ formatCurrency(saleQuote.netPayout || 0) }}</strong>
              </div>
            </div>
            ...
            <div class="sale-actions">
              ...
              <button
                ...
                data-focusable
              >
                {{ saleSubmitting ? 'Selling...' : 'Confirm Sale' }}
              </button>
            </div>
          </div>
        </div>
...
const openSaleModal = async () => {
  if (!props.data?.businessId || !props.data?.businessType || saleLoading.value) return
  saleLoading.value = true
  try {
    const quote = await lua.career_modules_business_businessManager.requestBusinessSaleData(
      props.data.businessType,
      props.data.businessId
    )
    if (quote?.success) {
      saleQuote.value = quote
...
const closeSaleModal = () => {
  ...
  saleQuote.value = null
}

const confirmSale = async () => {
  if (!props.data?.businessId || !props.data?.businessType || saleSubmitting.value) return
  saleSubmitting.value = true
  try {
    await lua.career_modules_business_businessManager.sellBusiness(
      props.data.businessType,
      props.data.businessId
    )
    showSaleModal.value = false
...
let saleModalPushedEl = null
watch(showSaleModal, (isOpen) => {
  if (!controllerNav) return
  if (isOpen) {
    nextTick(() => {
      if (saleModalRef.value) {
        controllerNav.pushModal(saleModalRef.value, closeSaleModal)
        saleModalPushedEl = saleModalRef.value
      }
    })
  } else if (saleModalPushedEl) {
    controllerNav.removeModal(saleModalPushedEl)
    saleModalPushedEl = null
  }
})
...
onUnmounted(() => {
  ...
  if (controllerNav && saleModalPushedEl) {
    controllerNav.removeModal(saleModalPushedEl)
    saleModalPushedEl = null
  }
