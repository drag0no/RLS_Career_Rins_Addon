<template>
  <div :class="['skill-tree-tab', { 'skill-tree-tab--multi': trees.length > 1 }]">
    ...
    <div v-else class="skill-tree-container">
      ...
      <p class="skill-tree-pan-hint">
        Right click + drag to move around, scroll wheel to zoom
      </p>
    </div>
    <Teleport to="body">
...
const TREE_NAME_VERTICAL_OFFSET_MULTI = 220

const getTreeNameHeaderLift = () =>
  trees.value.length > 1 ? TREE_NAME_VERTICAL_OFFSET_MULTI : TREE_NAME_VERTICAL_OFFSET
...
const getTreeNameStyle = (tree) => {
  ...
  const labelWidth = Math.max(width, NODE_WIDTH.value, 160)
  const top = Math.max(0, minY - getTreeNameHeaderLift())
...
const computeContentBounds = () => {
  ...
  trees.value.forEach(tree => {
    ...
    if (bounds) {
      ...
      minY = Math.min(minY, treeOffsetY + Math.max(0, localMinY - getTreeNameHeaderLift()))
...
.skill-tree-tab--multi .tree-name-label {
  font-size: 2.35em;
  white-space: normal;
}
