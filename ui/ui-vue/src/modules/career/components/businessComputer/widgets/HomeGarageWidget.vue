<template>
  <div class="home-widget garage-widget">
    ...
    <div class="widget-content" v-if="store.pulledOutVehicle">
      ...
      <div class="actions-row" v-if="!isPersonalVehicle">
        ...
        <button
          ...
          @click="$emit('abandon', currentVehicleJob)"
          :title="store.businessType === 'racingTeam' ? 'Sell vehicle' : 'Abandon Job'"
          data-focusable
        >
    ...
    <div class="widget-content empty" v-else>
      ...
      <button class="btn-link" @click="$emit('go-to-jobs')" data-focusable>
        {{ store.businessType === 'racingTeam' ? 'You must purchase a vehicle' : 'Select a vehicle from Jobs' }} 
      </button>
      </div>
  </div>
</template>
...
const currentVehicleJob = computed(() => {
  ...
  const vid = normalizeJobId(vehicle.vehicleId)
  return jobs.find(job => {
    const sv = normalizeJobId(job?.storedVehicleId)
    if (sv && vid && sv === vid) return true
    const jobId = job?.jobId ?? job?.id
    const vj = normalizeJobId(vehicle.jobId)
    return jobId !== undefined && jobId !== null && vj && String(jobId) === vj
  }) || null
})
