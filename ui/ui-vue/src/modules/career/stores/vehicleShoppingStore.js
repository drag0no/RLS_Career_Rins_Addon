...
export const useVehicleShoppingStore = defineStore("vehicleShopping", () => {
  ...
  /** Personal wallet or team business balance — whichever applies to the active shopping context. */
  const getFilterSpendCap = () => {
    const d = vehicleShoppingData.value
    if (d?.racingTeamBusinessMoney != null && Number.isFinite(Number(d.racingTeamBusinessMoney))) {
      return Number(d.racingTeamBusinessMoney) || 0
    }
    return Number(d?.playerAttributes?.money?.value ?? d?.playerAttributes?.money ?? 0) || 0
  }
  ...
  const isDealerHidden = (dealerId) => {
    ...
    // Once the backend confirms discovery, reputation controls checkout and
    // delivery privileges instead of hiding the dealer's inventory again.
    if (dealerMeta.undiscovered === false) return hidden

    // In-person at this seller (Lua currentSeller): skip org rep "offline" hiding so the lot is shoppable.
    // Racing team: taxi sets currentSeller after arrival — no global unlock of all dealers from the computer.
    const atThisSeller =
      d.currentSeller != null &&
      d.currentSeller !== undefined &&
      String(d.currentSeller) === String(dealerId)

    // Check organization's hiddenFromDealerList based on current reputation level
    if (!atThisSeller && dealerMeta.associatedOrganization && d.organizations) {
  ...
  const filteredVehicles = computed(() => {
    ...
    let filteredList = vehicleList.reduce(function (result, vehicle) {
      if (d.currentSeller != null && d.currentSeller !== undefined) {
        ...
        if (String(vehicle.sellerId) === String(d.currentSeller)) result.push(vehicle)
      } else {
  ...
  const updateFiltersOnOpen = () => {
    ...
    const spendCap = getFilterSpendCap()
    ...
    if (priceFilter && priceFilter.max !== undefined) {
      // If the current price filter max is higher than spend cap, update it
      if (priceFilter.max > spendCap) {
        const effectiveMax = Math.max(priceFilter.min || 0, Math.min(spendCap, priceFilter.max))
  ...
  // Watch spend cap (personal or team) and update "can afford" filter automatically
  watch(
    () => getFilterSpendCap(),
    (newCap, oldCap) => {
      ...
      const priceFilter = filters.value.Value
      if (priceFilter && priceFilter.max !== undefined) {
        if (newCap < oldCap && priceFilter.max > newCap) {
          const effectiveMax = Math.max(priceFilter.min || 0, Math.min(newCap, priceFilter.max))
          if (effectiveMax !== priceFilter.max) {
            setFilterRange('Value', priceFilter.min, effectiveMax)
          }
        }
