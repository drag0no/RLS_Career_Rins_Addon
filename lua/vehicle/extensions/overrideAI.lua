...
g = math.max(1e-30, math.abs(g)) -- prevent divivion by 0 gravity
...
local internalState = {
  ...
  road = 'onroad',
  crash = {time = 0, manoeuvre = 0, dir = nil, pos = nil, retargetRequested = false},
  derby = {
    recoveryTimer = 0,
    recoveryDuration = 0,
    recoveryDirection = -1,
...
local traffic = {
  ...
  -- Rfl / Rfl_fun / Rfs / Rfs_fun / distAhead: used when planAhead builds traffic.trafficTable with traffic.filtered. Racing turns the filter off and raceplanAhead uses a separate snapshot; these could be reused there later if we want distance-limited racing traffic awareness.
  Rfl = 48, -- larger radius for traffic filter queue
  Rfl_fun = function(v) return clamp((v+10)*3, 48, 140) end,
  Rfs = 45, -- smaller radius for traffic table
  Rfs_fun = function(v) return clamp(v*3, 45, 85) end,
  distAhead = 28 -- minimum Ahead distance for searching traffic vehicle
}
...
local speedProfile
local routeLoop = { loopPath = nil, noOfLaps = nil }
local parameters
...
local function getObjectBoundingBox(id)
  ...
  local y = z:cross(x)
  y:setScaled(obj:getObjectInitialWidth(id) * 0.5 / math.max(y:length(), 1e-30))
...
local function resetInternalStates()
  ...
  internalState.crash.retargetRequested = false
  internalState.crash.crashes = nil
  crashObjID = -1

  internalState.derby.recoveryTimer = 0
  internalState.derby.recoveryDuration = 0
  internalState.derby.recoveryDirection = -1
  internalState.derby.recoverySteer = 0
  internalState.derby.recoveryCooldown = 0
  internalState.derby.recoveryAttempts = 0
  internalState.derby.reason = nil
...
local function resetParameters()
  ...
  parameters = {
    ...
    -- Racing / competitive: aggressive overtaking (see raceplanAhead + alt plan selection).
    -- Defaults aligned with aiRacers post-.39 pass tuning (setParameters still wins at race GO).
    raceTrafficPassBlend = 0.78, -- 0..~0.85: recover this fraction of geometric vs traffic-capped target speed
    raceClearanceScaleMin = 0.26, -- lower = smaller effective rival OBB in racing (more overlap tolerance)
    raceAltPlanSpeedMargin = 6.5, -- larger = easier to commit to L/R offset plan on targetSpeed / traffic checks
    raceAltPlanTimeMargin = 3.8, -- larger = more path-length tolerance when choosing offset plan
    raceAwarenessCoefScale = 0.95, -- multiplies side displacement from rivals on racing line (lower = stay closer)
    -- When main plan has a blocker this far along (m), allow L/R if that side has more room or traffic target.
    raceBlockedAheadDistMax = 42,
    raceClearancePassMinGainM = 0.4, -- alt plan.distances must exceed main by at least this (m)
    raceClearancePassTrafficEps = 0.3, -- or alt trafficTargetSpeed exceeds main by this (m/s)
    -- Multiplies AI brake from speed error in racing (< 1 = softer corner / straight braking).
    raceBrakeGain = 0.82,
    raceCommitCorners = false,
    raceCornerGripUtil = 1.32,
    raceCornerPlanBoost = 1.06,
    raceStateBasedTcs = true,
    raceStateBasedThrottle = true,
    ...
    raceFrictionMult = 1.06,
    -- Chase closing contact: minimum relative crash speed (m/s). Police sets this from wanted heat.
    chaseCrashSpeed = 10,
  }
end
resetParameters()

local raceCommit, racingPlannerAccelMult, maxPropWheelSlip
do
  local rc = require('vehicle/extensions/overrideAI/raceCommit')
  raceCommit, racingPlannerAccelMult, maxPropWheelSlip = rc.build({
    opt = opt,
    parameters = parameters,
    ego = ego,
    targetSpeedSmoother = targetSpeedSmoother,
  })
end
...
-- Keep derby escape state separate from the regular route/crash state. Event-side
-- retargets can reset the latter, which used to cancel backing manoeuvres midway.
internalState.derby.beginRecovery = function(reason)
  if internalState.derby.recoveryTimer > 0 or internalState.derby.recoveryCooldown > 0 then return false end

  local previousDirection = internalState.derby.recoveryDirection
  internalState.derby.recoveryAttempts = math.min((internalState.derby.recoveryAttempts or 0) + 1, 4)
  internalState.derby.recoveryDuration = 1.35 + internalState.derby.recoveryAttempts * 0.3
  internalState.derby.recoveryTimer = internalState.derby.recoveryDuration
  internalState.derby.reason = reason or 'stuck'

  -- Forward attackers back out; reverse attackers pull forward. Alternating the
  -- steering bias prevents repeated straight-line attempts against the same wall.
  local preferredDirection = (M.mode == 'demoderby_reverse' or lastCommand.brake > lastCommand.throttle) and 1 or -1
  internalState.derby.recoveryDirection = internalState.derby.recoveryAttempts > 1 and -previousDirection or preferredDirection
  local targetSide = 0
  if player and player.pos then
    local awayFromTarget = ego.pos - player.pos
    if awayFromTarget:squaredLength() > 1e-6 then
      awayFromTarget:normalize()
      targetSide = ego.rightVec:dot(awayFromTarget)
    end
  end
  local alternateSide = internalState.derby.recoveryAttempts % 2 == 0 and -1 or 1
  internalState.derby.recoverySteer = clamp(targetSide * 1.5 + alternateSide * 0.45, -1, 1)

  internalState.egoCannotMoveTime = 0
  internalState.egoForceGoFrontTime = 0
  internalState.crash.time = 0
  internalState.crash.manoeuvre = 0
  internalState.crash.pos = nil
  internalState.crash.dir = nil
  ...
  obj:queueGameEngineLua("if gameplay_events_freeroam_demolitionDerby and gameplay_events_freeroam_demolitionDerby.onAiRecoveryRequest then gameplay_events_freeroam_demolitionDerby.onAiRecoveryRequest(" .. objectId .. ") end")
  return true
end

internalState.derby.checkCollision = function()
  if internalState.derby.recoveryTimer > 0 or internalState.derby.recoveryCooldown > 0 then return end
  mapmgr.getObjects()
  local selfObj = mapmgr.objects[objectId]
  if not selfObj or not selfObj.objectCollisions then return end

  for id, collisionState in pairs(selfObj.objectCollisions) do
    local candidate = mapmgr.objects[id]
    if collisionState == 1 and id ~= objectId and candidate and candidate.active == true then
      crashObjID = id
      internalState.derby.beginRecovery('impact')
      return
    end
  end
end
...
local function driveToTargetDerby(targetPos, throttle, brake)
  if not targetPos then return end

  local derbyState = internalState.derby
  if derbyState.recoveryTimer > 0 then
    derbyState.recoveryTimer = math.max(0, derbyState.recoveryTimer - dt)
    local progress = derbyState.recoveryDuration > 0 and derbyState.recoveryTimer / derbyState.recoveryDuration or 0
    local effort = 0.50 + 0.20 * progress
    local steering = derbyState.recoverySteer

    if derbyState.recoveryDirection < 0 then
      driveCar(steering, 0, effort, 0)
    else
      driveCar(-steering, effort, 0, 0)
    end

    if derbyState.recoveryTimer <= 0 then
      derbyState.recoveryCooldown = 0.9
      derbyState.reason = nil
      internalState.egoCannotMoveTime = 0
    end
    return
  end
  ...
  if player then
    ahead2 = player.pos + player.vel * dt
  else	
    ahead2 = targetPos
  end

  ahead = ego.dirVec * (-ego.length) + ego.pos
  local targetVec = ahead - ahead2; targetVec:normalize()
  local dirTarget = ego.dirVec:dot(targetVec)
  targetVec = -targetVec
  dirTarget = -dirTarget
  
  local aheadTarget = ego.dirVec:dot(targetVec)
  local dirDiff = -math.asin(ego.dirVec:cross(ego.upVec):normalized():dot(targetVec))
  local dirAngle = math.asin(ego.rightVec:dot(targetVec))
  local dirVel = ego.vel:dot(ego.dirVec)
  local absAiSpeed = math.abs(dirVel)
  
  if internalState.crash.manoeuvre and internalState.crash.manoeuvre > 0 then
    if not internalState.crash.pos and internalState.crash.manoeuvre == 4 then
      local crashObj = mapmgr.objects[crashObjID]
      if crashObj then
        internalState.crash.dir = -ego.dirVec
        internalState.crash.pos = crashObj.pos
      end
    end
    local distFromCrash
    if internalState.crash.pos then
      distFromCrash = ego.pos:distance(internalState.crash.pos)
    else
      -- Lost collision context, clear crash manoeuvre and continue with normal pursuit.
      internalState.crash.manoeuvre = 0
      internalState.crash.pos = nil
      internalState.crash.time = 0
      internalState.crash.crashes = nil
      crashObjID = -1
      if internalState.crash.retargetRequested then
        M.targetObjectID = -1
        player = nil
        internalState.crash.retargetRequested = false
      end
    ...
    if distFromCrash and (internalState.crash.manoeuvre == 5 and distFromCrash < 6 or internalState.crash.manoeuvre ~= 5 and distFromCrash > 8) then
      internalState.crash.manoeuvre = 0
      internalState.crash.pos = nil
      internalState.crash.time = 0
      internalState.crash.crashes = nil
    ...
    elseif distFromCrash then
      local crashTargetVec = (internalState.crash.pos - ahead):normalized()
      local crashTarget = internalState.crash.dir:dot(crashTargetVec)
      dirTarget = -crashTarget
      dirDiff = -math.asin(ego.dirVec:cross(vec3(obj:getDirectionVectorUp())):dot(crashTargetVec))
      if internalState.crash.manoeuvre == 3 then
        throttle = math.min(math.max(1 - (ego.pos - internalState.crash.pos):length() * 0.05, 0.5),1)
      else
        throttle = math.min(math.max(0.2 + (ego.pos - internalState.crash.pos):length() * 0.005, 0.3),0.4)
      end
      if internalState.crash.manoeuvre == 4 then
        internalState.crash.time = (internalState.crash.time or 0) + dt
      end
    end
  else
    if absAiSpeed > 0.2 then
      local avgWheelSpeed = 0
      local wheelCount = 0
      local minAbsWheelSpeed = math.huge
      local maxAbsWheelSpeed = 0
      local lwheels = wheels.wheels
      if lwheels then
        for i = 0, tableSizeC(lwheels) - 1 do
          local wd = lwheels[i]
          if not wd.isBroken then
            wheelCount = wheelCount + wd.isSpeedo
            local absWheelVel = math.abs(wd.angularVelocity * wd.radius)
            avgWheelSpeed = avgWheelSpeed + absWheelVel * wd.isSpeedo
            if wd.brakeTorque > 0 then
              minAbsWheelSpeed = math.min(minAbsWheelSpeed, absWheelVel)
            end
            if wd.isPropulsed then
              maxAbsWheelSpeed = math.max(maxAbsWheelSpeed, absWheelVel)
            end
          end
        end
      end
      ...
      if wheelCount > 0 then
        local latVgrad = 0.5 * ego.width * math.abs(sensors.gx2) / (ego.speed + 1e-30)
        avgWheelSpeed = avgWheelSpeed / wheelCount

        -- abs
        local minGradSpeed = absAiSpeed - latVgrad
        if avgWheelSpeed < minGradSpeed and brake > 0 then
          coef = coef * minAbsWheelSpeed / (minGradSpeed + 1e-30)
        end

        -- tcs
        local maxGradSpeed = math.max(3, absAiSpeed + latVgrad)
        if maxAbsWheelSpeed > maxGradSpeed and throttle > 0 then
          local redcoef = maxGradSpeed / maxAbsWheelSpeed
          coef = coef * redcoef * redcoef
        end
      end
    end
    ...
    if ego.speed > 1 then
      ...
      local rotVel = math.min(1, (ego.prevDirVec:projectToOriginPlane(ego.upVec):normalized() - ego.dirVec):length() * dt * 10000)
      coef = coef * math.max(0, 1 - math.abs(rightVel * ego.speed * 0.05) * math.min(1, dirAngle * dirAngle * ego.speed * 6) * rotVel)
      local dirPer = math.abs(dirDiff) / 1.55
      if dirPer > .70 then
        local temp = 1 - dirPer
        coef = math.min(math.max(temp, coef),1)
      end
    end
  end
  
  internalState.egoForceGoFrontTime = math.max(0, internalState.egoForceGoFrontTime - dt)

  if not internalState.crash.crashes then internalState.crash.crashes = 1 end
  if internalState.crash.crashes > 3 then
    internalState.crash.crashes = 1
  end

  if internalState.egoCannotMoveTime > internalState.crash.crashes and internalState.egoForceGoFrontTime == 0 or (internalState.crash.time and internalState.crash.time > 8) then
    internalState.egoForceGoFrontTime = 5
    internalState.crash.crashes = internalState.crash.crashes + 1
  end
  
  if internalState.egoForceGoFrontTime > 0 then
    if dirTarget > 0 then
      dirTarget = -dirTarget		
    else
      dirTarget = math.abs(dirTarget)	
    end
    internalState.crash.time = 0
    internalState.egoCannotMoveTime = 0
  end
  
  if dirTarget < -.75 or ego.speed < 3 and dirTarget <= 0 then
      brake = throttle
      throttle = 0 
  elseif dirTarget > .85 or ego.speed < 3 and dirTarget >= 0 then
      throttle = throttle
      brake = 0 
  else
      if ego.speed < 3 then
  ...
  driveCar(-dirDiff, throttle * coef, brake * coef, pbrake)
end
...
local function driveToTarget(targetPos, throttle, brake, targetSpeed)
  ...
  if ego.speed > 1 then
    ...
    if rightVel * ego.rightVec:dot(targetPos - ego.pos) > 0 then
      local rotVel = math.min(1, (ego.prevDirVec:projectToOriginPlane(ego.upVec):normalized()):distance(ego.dirVec) * dt * 10000)
      throttleOverCoef = math.max(0, 1 - math.abs(rightVel * ego.speed * 0.05) * math.min(1, dirAngle * dirAngle * ego.speed * 6) * rotVel)
    end
  end

  local dirVel = ego.vel:dot(ego.dirVec)
  local absegoSpeed = math.abs(dirVel)
  ...
  if plan and plan[3] and dirVel > 3 then
    ...
    if outDeviation > 0 and absegoSpeed > 3 then
      local steerCoef = outDeviation * absegoSpeed * absegoSpeed * math.min(1, dirAngle * dirAngle * 4)
      local understeerCoef = math.max(0, steerCoef) * math.min(1, math.abs(ego.vel:dot(p2p1DirVec) * 3))
      if opt.racing and parameters.raceCommitCorners ~= false then
        local slipMin = (type(parameters.raceUndersteerSlipMin) == 'number' and parameters.raceUndersteerSlipMin) or 1.05
        if maxPropWheelSlip() < slipMin then
          understeerCoef = 0
        end
      end
      ...
      if opt.racing and type(parameters.raceWideLineUndersteerRelax) == 'number' and parameters.raceWideLineUndersteerRelax > 0 and plan[plan.egoSeg or 1] and plan[(plan.egoSeg or 1) + 1] then
        ...
        local onL = a + (b - a) * xn
        local latMag = math.abs((ego.pos - onL):dot(plan[es].normal))
      ...
      if not (opt.racing and parameters.raceCommitCorners ~= false and understeerCoef == 0) then
        brakeUnderCoef = math.min(brakeUnderCoef, math.max(0, 1 - understeerCoef * understeerCoef))
      end
    end
  else
  ...
  if absegoSpeed > 0.05 then
    if sensors.gz <= 0.1 then
      ...
      absegoSpeed = math.max(absegoSpeed, 3)

      totalSlip = totalSlip / (totalDownForce + 1e-25)

      -- abs
      brakeABSCoef = math.min(1, 1.5 * square(square(square(math.max(0, absegoSpeed - totalSlip) / absegoSpeed))))

      -- tcs
      propSlip = propSlip * (parameters.driveStyle == 'offRoad' and 0.8 or 1)
      local tcsCoef
      if opt.racing then
        local stateBased = parameters.raceStateBasedTcs ~= false
        if stateBased then
          local slipTarget = (type(parameters.raceSlipTarget) == 'number' and parameters.raceSlipTarget) or 1.14
          local slipGain = (type(parameters.raceSlipGain) == 'number' and parameters.raceSlipGain) or 0.26
          if propSlip <= slipTarget then
            ...
            tcsCoef = math.max(0, 1 - reduction)
            throttleTcsCoef = math.min(tcsCoef, smoothTcs:get(tcsCoef, dt))
          end
        else
          ...
          if propSlip <= slipThreshold then
            ...
            local reduction = math.min(maxReduction, slipGain * (propSlip - slipThreshold))
            tcsCoef = math.max(0, 1 - reduction)
            throttleTcsCoef = math.min(tcsCoef, smoothTcs:get(tcsCoef, dt))
          end
        end
      else
        tcsCoef = math.max(0, absegoSpeed - propSlip * propSlip) / absegoSpeed
        throttleTcsCoef = math.min(tcsCoef, smoothTcs:get(tcsCoef, dt))
      end
    else
      brakeABSCoef = 0
  ...
  if parameters.underSteerBrakeControl ~= 'off' then
    brakeCoef = math.min(brakeCoef, brakeUnderCoef)
  end
  if parameters.abBrakeControl ~= 'off' then
    brakeCoef = math.min(brakeCoef, brakeABSCoef)
  end

  local throttleCoef = 1
  if parameters.oversteerThrottleControl ~= 'off' then
    throttleCoef = math.min(throttleCoef, throttleOverCoef)
  end
  if parameters.understeerThrottleControl ~= 'off' then
    throttleCoef = math.min(throttleCoef, throttleUnderCoef)
  end
  if parameters.throttleTcs ~= 'off' then
    throttleCoef = math.min(throttleCoef, throttleTcsCoef)
  end
  ...
  if opt.racing and type(parameters.raceThrottleFloor) == "number" then
    ...
    if throttleTcsCoef >= floor then
      throttleCoef = math.max(throttleCoef, floor)
    end
  end
  ...
  if parameters.driveStyle == 'offRoad' then
    brakeCoef = 1
    throttleCoef = math.sqrt(throttleCoef)
  end
  ...
  if currentRoute and (dirTarget < 0 or (twt.state == 1 and dirTarget < 0.866)) then   -- TODO: Improve entry condition when twt.state == 1
    ...
    local tmpVec = vec3()
    local RRPicks = math.max(1, math.min(math.floor(150 * dt + 0.5), 8)) -- x * dt where x/fps is the numer of samples
    for k = 1, RRPicks do --the ray cast loop: each iteration scans one area and casts on the minimum
      ...
      if blueIndex < 3 then
        i = math.floor(1 + blueIndex)
      elseif blueIndex < 6 then
        i = math.floor(1 + blueIndex) + 1
      ...
      helperVec:normalize()
      local rayLen = castRay(tmpVec, helperVec, math.min(twt.rayMins[i], rayDist))
      if twt.rayMins[i] > rayLen or rayLen == rayDist then
        ...
        helperVec:normalize()
        twt.rayMins[i] = castRay(tmpVec, helperVec, math.min(math.max(2 * twt.rayMins[i], 0.02), rayDist))
      end
    ...
    local minRayDist = math.min(twt.rayMins[1], twt.rayMins[2], twt.rayMins[3], twt.rayMins[4], twt.rayMins[5], twt.rayMins[6], twt.rayMins[7], twt.rayMins[8])

    twt.sampleCounter = twt.sampleCounter + 1
    if twt.state == 0 then
      if twt.sampleCounter * dt > math.max(0.5, math.min(50 * dt, 2.5)) and ego.speed <= 0.5 then -- TODO: Optimize sample count condition
        twt.state = 1
      else
        local speed = math.max(0, ego.speed - 0.25)
        local throttle = math.max(0, -sign(speed * dirVel)) * 0.3
        local brake = math.max(0, sign(speed * dirVel)) * 0.3
    ...
    if twt.state == 1 then  -- start driving
      if twt.targetSpeed < 0.66 or twt.minRay == "S" then -- TODO: Some branches may be consolidated
        ...
        elseif minRayDist == twt.rayMins[6] then
          ...
          twt.dirState[2] = sign2(dirTargetAxis)
          twt.minRay = "B"
        elseif minRayDist == math.min(twt.rayMins[4], twt.rayMins[8]) then
          local sideRuler = 0
      ...
      local threshold = math.min(ego.speed - 0.1, 0.66)
      local dirCoef, minDist = 1, nil --TODO: what is car is trapped/sandwiched?
      if twt.dirState[1] == -1 then -- reverse
        dirCoef = math.min(1.2 - dirTarget, 1) -- dirTarget -> targetSpeed modulation
        if twt.dirState[2] == -1 then -- steer left
          minDist = math.min(twt.rayMins[4], twt.rayMins[6], twt.rayMins[7])
        elseif twt.dirState[2] == 1 then -- steer right
          minDist = math.min(twt.rayMins[5], twt.rayMins[6], twt.rayMins[8])
        else -- no steering
          minDist = twt.rayMins[6]
          if twt.minRay ~= "S" then
            threshold = twt.rayMins[2] -- possibly math.max(threshold, twt.rayMins[2])
          end
        end
      else -- forward
        if twt.dirState[2] == -1 then
          minDist = math.min(twt.rayMins[1], twt.rayMins[2], twt.rayMins[8])
        elseif twt.dirState[2] == 1 then
          minDist = math.min(twt.rayMins[2], twt.rayMins[3], twt.rayMins[4])
        else
          minDist = twt.rayMins[2]
        end
      end

      local targetSpeed = math.sqrt(2 * g * math.min(aggression, ego.staticFrictionCoef) * math.max(0, minDist - threshold) * dirCoef)
      local stateBasedRacing = opt.racing and parameters.raceStateBasedTcs ~= false
      ...
      local speedCap = opt.racing and math.min(capMax, aggression * capAgg) or math.min(6, aggression * 6)
      twt.targetSpeed = math.max(math.min(twt.speedSmoother:get(targetSpeed, dt), speedCap), 0.3)
    ...
    if ego.vel:dot(ego.dirVec) < 0 and ego.speed > 0.1 then
      ...
      -- Racing: avoid parking brake on sharp turn-in (causes harsh corner braking); keep only near-stop behavior.
      local cornerHandbrake = (ego.speed > 4 and ego.speed < 30 and math.abs(dirAngle) > 0.97 and brake == 0)
      if opt.racing then
        cornerHandbrake = false
      end
      if cornerHandbrake or (ego.speed < 0.15 and targetSpeed <= 1e-5) then
    ...
    local aggSq = square(aggression + math.max(0, -(ego.dirVec:dot(gravityDir))))
    local stateBasedRacing = opt.racing and parameters.raceStateBasedThrottle ~= false
    ...
    local rate = math.max(throttleSmoother[throttleSmoother:value() < throttle], 10 * aggSq * aggSq * rateMult * recoveryMult)
...
local function egoPosOnPlan(plan)
      ...
      if i < planCount - 2 then
        ...
        if nextXnorm >= 0 then
          ...
          if ego.pos:squaredDistance(linePointFromXnorm(p1Pos, plan[i+2].pos, nextXnorm)) <
              square(ego.width + lerp(p1Radius, plan[i+2].radiusOrig, math.min(1, nextXnorm))) then
  ...
  if egoSeg > 1 then
    ...
    if not planNodeStack[25] then
      table.insert(planNodeStack, plan[1])
    end
    ...
    plan.planLen = math.max(0, plan.planLen - sumLen)
    plan.stopSeg = math.max(1, plan.stopSeg - disp)
  end
...
local function calculateTarget(plan)
  egoPosOnPlan(plan)
  local targetLength = math.max(ego.speed * parameters.lookAheadKv, parameters.minStTargetDist)

  if plan.planCount >= 3 then
    local xnorm = clamp(plan.egoXnormOnSeg, 0, 1)
    targetLength = math.max(targetLength, plan[1].length * (1 - xnorm), plan[2].length * xnorm)
  end

  local remainder = targetLength

  local targetPos = vec3(plan[plan.planCount].pos)
  local targetSeg = math.max(1, plan.planCount-1)
...
local function targetsCompatible(baseRoute, newRoute)
  ...
  return math.abs(newTvec:normalized():dot(baseTargetRight)) * ego.speed < 2
end
...
local function getPlanLen(plan, from, to)
  ...
  to = math.min(plan.planCount-1, to or math.huge)
  local planLen = 0
  for i = from, to do
...
local function updatePlanLen(plan, j, k)
  ...
  j = math.max((j or 1) - 1, 1)
  k = math.min(k or plan.planCount, plan.planCount-1)
...
local function openPlanLanesToLaneRange(plan, idx)
  ...
  if not plan then return end
  for i = 1, math.min(plan.planCount, idx) do
...
local function laneChange(plan, dist, signedDisp)
  ...
  for i = 2, plan.planCount do
    ...
    plan[i].lateralXnorm = clamp(plan[i].lateralXnorm + signedDisp * math.min(curDist * invDist, 1), plan[i].laneLimLeft, plan[i].laneLimRight)
...
local function numOfLanesFromRadius(rad1, rad2)
  return math.max(1, math.floor(math.min(rad1, rad2 or math.huge) * 2 / 3.61 + 0.5)) -- math.floor(math.min(rad1, rad2) / 2.7) + 1
end
...
local function getEdgeLaneConfig(fromNode, toNode)
  ...
  if edge.lanes then
    ...
    if edge.oneWay then
      ...
      local numOfLanes = math.max(1, math.floor(numOfLanesFromRadius(mapData.radius[fromNode], mapData.radius[toNode]) * 0.5))
...
local function roadNaturalContinuation(wp1, wp2)
  ...
  local laneFlow = mapData.graph[wp1][wp2].drivability * 4 * math.min(inRadiuswp2, inRadiuswp1) / #inLaneConfig
  ...
  for k, v in pairs(mapData.graph[wp2]) do
    if k ~= wp1 then
      ...
      local dirCoef = 0.5 * math.max(0, 1 + outEdgeDir:dot(inEdgeDir))
      ...
      if numOfOutLanes == inLaneCount and  outLaneCountOpposite == inLaneCountOpposite then
        ...
        laneFlow = mapData.graph[wp2][k].drivability * 4 * math.min(outRadiuswp2, outRadiusk)/ #outLaneConfig
      end
      local outFwdFlow = math.min(inFwdFlow, numOfOutLanes * laneFlow)
      local outBackFlow = math.min(inBackFlow, outLaneCountOpposite * laneFlow)
...
local function processNodeIncomingLanes(newNode, planNode)
  ...
  local defaultLaneWidth = math.min(2 * halfWidth / (#newNode.inEdgeLanes), 3.45)
  ...
  for nodeId, edgeData in pairs(graph[newNode.id]) do
    if nodeId ~= newNode.nextNodeInPath and nodeId ~= prevNodeInPath then -- and not mapmgr.signalsData.nodes[newNode.id]
      ...
      if thisEdgeInLanes > 0 then -- TODO: Should also possibly include traffic light data
        ...
        local dirRatio = math.max(0, (push3(mapData.positions[newNode.nextNodeInPath or newNode.id]) - newNode.posOrig):normalized():dot((push3(newNode.posOrig) - nodePos):normalized()))
        local drivabilityRatio = math.min(1, edgeData.drivability * inEdgeDrivabilityInv)
        ...
        if thisEdgeInLanes > 0 then
          ...
          elseif numOfInLanes < numOfOutLanes then
            local capInLanes = math.min(numOfInLanes + thisEdgeInLanes, numOfOutLanes)
...
local function getBestLane(laneConfig, nodeLatPos, laneLeftLimLatPos, laneRightLimLatPos, plan, newNode)
  ...
  local numOfLanes = math.max(1, #laneConfig)

  local newEdgeVec, newNodeWidthVec, dirVec = vec3(), vec3(), vec3()
  if plan then
    dirVec:setSub2(plan[plan.planCount].pos, plan[math.max(1, plan.planCount-1)].pos); dirVec:normalize()
  ...
  for i = 1, numOfLanes do -- traverse lanes in laneConfig
    ...
    local colinearityError = 1 - math.max(0, laneColinearity)
    local distError = math.abs(thisLaneLatPos - nodeLatPos)
    local overlapError = laneWidth - math.max(0, math.min(thisLaneLimRight, laneRightLimLatPos) - math.max(thisLaneLimLeft, laneLeftLimLatPos))
  ...
  elseif not wp3 then
    ...
    if e1P2:squaredDistance(e2P1) < 0.005 then
      ...
      elseif e2Xnorm < 1 and e1Xnorm >= 0 and (e1Xnorm - 1) * e1P2:distance(e1P1) <= e1R2 then
        ...
        return linePointFromXnorm(e1P1, e1P2, math.max(e1Xnorm, 1 - e1R2/segLen))
      ...
      elseif e1Xnorm > 0 and e2Xnorm * e2P1:distance(e2P2) >= -e2R1 and e2Xnorm <= 1 then
        ...
        return linePointFromXnorm(e2P1, e2P2, math.min(e2Xnorm, e2R1/segLen))
      ...
      elseif e2Xnorm >= 1 or e1Xnorm < 0 then
        ...
        return avgPoint + math.max(0, math.min(1, (e1R2 + e2R1) * 0.5 / avgPointToP1Line:length())) * avgPointToP1Line
      end
    end
  end
end
...
local function getNewNode()
  local node = table.remove(planNodeStack)
  if node then
    node.wp = nil
  else
    node = table.new(0, 30)
...
local function buildNextRoute(route)
  ...
  if routeLoop.loopPath == true and routeLoop.noOfLaps and routeLoop.noOfLaps > 1 and not path[nextPathIdx] then -- in case the path loops
    local loopPathId
    local pathCount = #path
    ...
    routeLoop.noOfLaps = routeLoop.noOfLaps - 1 -- avoid decreasing noOfLaps for alternative plans
  end
  ...
  if planCount > 1 then
    if plan[planCount].nextNodeInPath ~= path[nextPathIdx] then
      ...
      local chordLength = math.min(2 / tmp, (1 - norm1:dot(norm2) * 0.5) * tmp) -- 2 / tmp == cos(x/2) : x is angle between norm1, norm2
  ...
  local newNodeChordLength = math.min(2 / tmp, (1 - newNode.inEdgeNormal:dot(newNode.outEdgeNormal) * 0.5) * tmp) -- new node road width multiplier
  ...
  if opt.driveInLaneFlag and newNode.inEdgeLanes then
    ...
    local tmp = newNode.normal:length()
    newNode.normal:setScaled(1 / (tmp + 1e-30))
    newNodeChordLength = math.min(2 / tmp, (1 - newNode.inEdgeNormal:dot(newNode.outEdgeNormal) * 0.5) * tmp)
    newNode.halfWidth = newNode.radiusOrig * newNodeChordLength
  end
  ...
  if newNodeLaneConfig and #newNodeLaneConfig > 1 and newNode.nextNodeInPath and newNode.prevNodeInPath then -- TODO: error hits if the nextNode/prevNode conditions are not here
    ...
    if (push3(mapData.positions[newNode.nextNodeInPath]) - newNode.posOrig + mapData.positions[newNode.prevNodeInPath] - newNode.posOrig):dot(newNode.normal) < 0 then -- = turnDir * normal
      ...
      newNode.rangeLeft = math.min(newNodeRangeLeft, linearScale(-newNodeHalfWidth + (newNodeRangeLeftIdx - 1) * laneWidth, -newNodeHalfWidth, newNodeHalfWidth, 0, 1))
      --]]
  ...
  if not opt.driveInLaneFlag then
    ...
    local lateralXnorm = math.max(math.min(normalizedPrevLatXnorm * newNode.halfWidth, newNode.halfWidth), -newNode.halfWidth)

    -- calculate smoothing parameter
    local t = math.min(1, math.max(0, plan.planLen / 100)) -- currently converges to the center of the road at 100m
...
local function uniformPlanErrorDistribution(plan)
  if twt.state == 0 then
    ...
    dispVec:setScaled(math.min(1, 2 * dt))
...
local function inCurvature(v1, v2)
  ...
  local cosSq = math.min(1, dot12 * dot12 / math.max(1e-30, v1Sqlen * v2Sqlen))

  if dot12 < 0 then -- angle between the two segments is > 90 deg
    local minDsq = math.min(v1Sqlen, v2Sqlen)
    local maxDsq = minDsq / math.max(1e-30, cosSq)
    if math.max(v1Sqlen, v2Sqlen) > (minDsq + maxDsq) * 0.5 then
      if v1Sqlen > v2Sqlen then
        -- swap v1 and v2
        v1x, v1y, v1z, v2x, v2y, v2z = v2x, v2y, v2z, v1x, v1y, v1z
        v1Sqlen, v2Sqlen = v2Sqlen, v1Sqlen
      end
      local s = math.sqrt(0.5 * (minDsq + maxDsq) / math.max(1e-30, v2Sqlen))
      v2x, v2y, v2z = s * v2x, s * v2y, s * v2z
    end
  end

  return 2 * math.sqrt((1 - cosSq) / math.max(1e-30, square(v1x + v2x) + square(v1y + v2y) + square(v1z + v2z))) -- the denominator is v1:squaredLength(-v2)
end
...
local function speedBasedAccelBudget(x, a)
  ...
  return math.max(0, math.min(fx + a, ego.staticFrictionCoef * g))
end
...
local function acc_eval_1(speedSq, acc_max, curvature)
  ...
  return ax_max_tyre * math.max(0, 1 - ay_used / ay_max_tyre)
end
...
local function acc_eval_2(speedSq, acc_max, curvature)
  ...
  if ay_used < ay_max_tyre then
    return ax_max_tyre * math.sqrt(1 - square(ay_used/ay_max_tyre))
  else
    return 0
  end
end
...
local function solver_f_acc_profile(plan, speed0)
  ...
  for i = 1, plan.planCount-1 do
    ...
    if plan.stopSeg <= i+1 then
      ...
      if math.min(n2.speed, n2.trafficSqVel) < n1.speed then -- max velocity at i-1 is less than velocity at i (not a deceleration phase)
        vx_possible_next = math.min(n2.speed, n2.trafficSqVel)
      else
        local ax_final = acc_eval(n1.speed, n1.acc_max, math.abs(n1.curvature))
        vx_possible_next = n1.speed + 2 * ax_final * n1.length -- speed squared
        vx_possible_next = math.min(n2.speed, math.min(n2.trafficSqVel, vx_possible_next))
      end
    end
  ...
  plan[plan.planCount].speed = math.sqrt(plan[plan.planCount].speed)
end
...
local function solver_b_acc_profile(plan)
  for i = plan.planCount, 2, -1 do
    ...
    if plan.stopSeg <= i-1 then
      ...
      if math.min(n1.speed, n1.trafficSqVel) < n2SpeedSq then -- max velocity at i-1 is less than velocity at i (not a deceleration phase)
        vx_possible_next = math.sqrt(math.min(n1.speed, n1.trafficSqVel))
      else
        -- available longitudinal acceleration at node i
        local ax_possible_current = acc_eval(n2SpeedSq, n2.acc_max, math.abs(n2.curvature))
        ...
        local ax_possible_next = acc_eval(vx_possible_next, n1.acc_max, math.abs(n1.curvature))
        ...
        if vx_possible_next > vx_tmp then
          ...
          ax_possible_next = acc_eval(vx_tmp, n1.acc_max, math.abs(n1.curvature))
          ...
          vx_possible_next = math.min(vx_possible_next, vx_tmp)
        end

        vx_possible_next = math.sqrt(math.min(n1.speed, math.min(n1.trafficSqVel, vx_possible_next))) -- respect traffic speed
      end
    end

    n1.speed = n1.manSpeed or
               (M.speedMode == 'limit' and M.routeSpeed and math.min(M.routeSpeed, vx_possible_next)) or
               (M.speedMode == 'set' and M.routeSpeed) or
               vx_possible_next

    if M.speedMode == 'legal' then
      n2.legalSpeed = n2.legalSpeed or n2.speed
      n1.legalSpeed = math.min(math.min(n1.speed, math.sqrt(n2.legalSpeed * n2.legalSpeed + (n1.acc_max + n2.acc_max) * n1.length)), (n1.roadSpeedLimit or math.huge))
    end
...
local function trafficFilter(index, route, v, draw)
  ...
  for i = math.max(index.start, plan2.pathidx), index.final do
    ...
    local minF = math.min(xnormFrRight, xnormFrLeft)
    local minR = math.min(xnormReRight, xnormReLeft)
    ...
    if (minF > 0 and minF < 1) or (minR > 0 and minR < 1) then
      ...
      if ego2PlVec:dot(vdirVec) < 0 then
        ...
        if minF < 1 and minR > 1 and path[i+1] then
          ...
          dirVec:setAdd2(math.max(0,minF)*dirVecNext, math.max(0,1-minF)*dirVec)
...
local function calculateTrafficTargetSpeed(plan, trafficTable)
  ...
  if trafficTableLen > 0 and plan.targetSpeed > 0 then
    ...
    for i = 1, plan.planCount - 1 do
      ...
      local sk = math.floor(segC.x) .. "|" .. math.floor(segC.y) .. "|" .. math.floor(surfRad * 100 + 0.5)
      ...
      for j = trafficTableLen, 1, -1 do
        ...
        plan.distancesV = check_ahead and math.min(ego.pos:squaredDistance(v.posMiddle), plan.distancesV) or plan.distancesV
        ...
        if overlapsOBB_OBB(v.posMiddle, v.vx, v.vy, v.vz, segC, segVx, segVy, segVz) then
          ...
          --plan.distances = math.min(distOnPlan, plan.distances)
          -- if the vehicle overlaps the first segment check if its xnorm is greater than the xnorm of the ego vehicle
          if i > 1 or math.max(vRearXnorm, vFrontXnorm) > plan.egoXnormOnSeg then
            local vXnorm = clamp(math.min(vRearXnorm, vFrontXnorm), 0, 1)
            --local ego2PlDist = math.max(0, distOnPlan - plan[1].length * plan.egoXnormOnSeg + clamp(vXnorm, 0, 1) * n1.length) -- if current segment length is added after the traffic table loop
            ...
            local ego2PlDist_0 = math.max(0, distOnPlan - (1 - vXnorm) * n1.length) --math.max(2.5, opt.racing and ego.speed*0.5 or ego.speed * 1)
            plan.distances = math.min(ego2PlDist_0, plan.distances)

            ego.race.d = 0.5*ego.length
            local ego2PlDist = ego2PlDist_0 - ego.speed * ego.race.time_gap --math.max(2.5, ego.speed * 1)
            local velProjOnSeg = v.vel:dot(segDirVec) -- math.max(0, v.vel:dot(segDirVec))
            local gain = (ego2PlDist > 0 and ego.race.catchAgg or ego.race.brakeAgg) * math.abs(ego2PlDist) + 1 -- gain factor to try to catch the vehicle ahead by boosting acceleration (ego2PlDist > 0) or brake at maximum acceleration (ego2PlDist < 0)
            local targetSpeed = math.max(0, math.abs(velProjOnSeg) * velProjOnSeg + 2 * g * math.min(aggression, ego.staticFrictionCoef) * ego2PlDist * gain)
            ...
            plan.trafficTargetSpeed = math.min(targetSpeed, plan.trafficTargetSpeed)
            plan.trafficMinProjSpeed = math.min(velProjOnSeg, plan.trafficMinProjSpeed)
          end
  ...
  local TTC = plan.distances / math.max(dv, 1e-3)
  if TTC < 3 and math.abs(plan.trafficMinProjSpeed) < 5 then
    plan.trafficTargetSpeed = 0
  end
  plan.trafficTargetSpeed = math.sqrt(plan.trafficTargetSpeed)
  ...
  --  --ego.race.d = (square(ego.speed) - square(plan.trafficMinProjSpeed))/(math.min(aggression, ego.staticFrictionCoef) * 2 * g)

  --  -- logic for emergency brake (it is not working do to false flag, need some improvements on TTC value)
  --  local TTC = plan.distances / math.max(dv, 1e-3)
  --  if TTC < 2 and math.abs(plan.trafficMinProjSpeed) < 3 then
  --    plan.trafficTargetSpeed = 0
  --  else
  ...
  --    plan.trafficTargetSpeed = plan.trafficMinProjSpeed + boost - ego.race.kp * math.max(0, dv)
  --  end
  --  plan.trafficTargetSpeed = math.max(plan.trafficTargetSpeed, 0)
  --  --dump(plan.trafficMinProjSpeed, plan.distances, dv, plan.trafficTargetSpeed, ((plan.distances - ego.race.d) / ego.race.Td), - ego.race.kp * math.max(0, dv))
  --else
  --  plan.trafficTargetSpeed = math.sqrt(plan.trafficTargetSpeed)
  --end
  ...
  --  ego.race.d = min_gap
  --  local boost = math.max(0, (plan.distances - targetFollowDist)/ego.race.Td) -- boost factor to try to catch the vehicle ahead

  --  local breaking_dist = math.max(0, (square(ego.speed) - square(plan.trafficMinProjSpeed))/(math.min(aggression, ego.staticFrictionCoef) * 2 * g))
  ...
  --    dynamic_kp = ego.race.kp * math.sqrt( (min_gap * 3) / math.max(safety_margin, 1e-3) )
  --  end

  --  local damping = dynamic_kp * math.max(0, dv)
...
local function copyPlan(tab2copy, tab2fill)
  ...
  local keysNum = 32
  for i = 1, math.max(#newPlan, #tab2copy) do
...
local function side_avoidance(plan)
  ...
  if #traffic.trafficTable > 0 then
    ...
    for _, v in ipairs(traffic.trafficTable) do
      ...
      local v_close = math.max(0, -sign(dD) * v_rel) -- Approaching speed, positive if approaching, zero otherwise
      ...
      local gap_free = math.max(0, math.abs(dD) - half_sum) -- compute current free gap
      local deficit = math.max(0, c_req - gap_free) -- compute missing free gap
      local contrib_mag = a * deficit -- scale missing free gap by intensity factor
      if contrib_mag > 0 then
        if dD >= 0 then
          dispLeft = contrib_mag > 0.05 and math.max(contrib_mag, dispLeft) or 0
        else
          dispRight = contrib_mag > 0.05 and math.max(contrib_mag, dispRight) or 0
        end
      end
    end
...
local function calculateFittingError(plan)
  ...
  plan[1].fE = 0
  for i = 2, plan.planCount - 1 do
    local r = 1 / (math.abs(plan[i].curvature) + 1e-100)
    plan[i-1].fE = math.max(plan[i-1].fE, r - math.sqrt(math.max(0, r * r - square(0.5 * plan[i-1].length))))
    plan[i].fE = r - math.sqrt(math.max(0, r * r - square(0.5 * plan[i].length)))
  end
end
...
local function densifyPlan(plan, path)
  ...
  for i = 1, plan.planCount-1 do
    if distOnPlan < 20 then
      plan[i].rfe = math.max(plan[i].rfe, 1 - math.sqrt(math.max(0, 1 - square(0.5 * plan[i].length * plan[i+1].curvature))))
    end
    if plan[i].length > math.max(0, 0.5 * (math.max(distOnPlan, 0) - math.max(ego.speed * parameters.str, 20))) + math.max(ego.speed * parameters.stt, 6) or distOnPlan < 20 and plan[i].length > 4 and plan[i].rfe > 0.045 then -- or plan[i].length > 4.5 and distOnPlan < 30 and plan[i].fE > 0.3
      local n1, n2 = plan[i], plan[i+1]
      ...
      local limPos = linePointFromXnorm(plan[i].posOrig + plan[i].radiusOrig * edgeNormal, plan[i+1].posOrig + plan[i+1].radiusOrig * edgeNormal, math.max(0, math.min(1, t2)))
    ...
    if distOnPlan < 20 then
      plan[i+1].rfe = 1 - math.sqrt(math.max(0, 1 - square(0.5 * plan[i+1].length * plan[i+1].curvature)))
    end
  end
end
...
local function raceplanAhead(route, baseRoute, pmode)
      ...
      if commonPathEnd >= 1 then
        ...
        for i = 1, #bsrPlan do
          ...
          plan[i] = {
            ...
            pathidx = math.max(1, n.pathidx-refpathidx),
  ...
  if not route.pathLength[#route.path] and not pmode then
    ...
    route.pathLength[n+1] = mapData.positions[route.path[n+1]]:distance(mapData.positions[route.path[math.max(1, n)]]) + route.pathLength[n]
  end
  ...
  if opt.racing then
    local cmin = (type(parameters.raceClearanceScaleMin) == 'number' and parameters.raceClearanceScaleMin) or 0.5
    cmin = clamp(cmin, 0.22, 0.95)
    plan.clearanceScale = math.min(math.max(1 - 0.5 * aggression, cmin), 1)
  end
  ...
  if not pmode then
    ...
    if plan.dispLat ~= 0 then
      ...
      sideDisp = math.min(dt * awarenessCoef * 10, math.abs(sideDisp)) * sign2(sideDisp) -- limited displacement per frame
      if opt.racing and plan.clearanceScale then
        sideDisp = sideDisp * plan.clearanceScale
      end
      local curDist = 0
      local lastPlanIdx = 2
      local targetDist = square(ego.speed) / (2 * g * aggression) + math.max(30, ego.speed * 3) -- longer adjustment at higher speeds
      local tmpVec = vec3()
      for i = 2, plan.planCount - 1 do
        openLaneToLaneRange(plan[i])
  ...
  for i = 1, plan.planCount-1 do
    ...
    nforce:setScaled2(n1.turnDir, (1-twt.state) * math.max(1 - v1:dot(v2), 0) * turnCoef)
  ...
  for i = 2, plan.planCount-skipLast do
    ...
    local displacement = math.max(math.min(n.normal:dot(forces[i]), forceMagLim), -forceMagLim)
    displacement = displacement * math.max(math.min(displacement * n.dispDir * math.huge, 1), 0) -- second term returns 0 if (displacement * n.dispDir) is negative, 1 otherwise.
    n.dispDir = sign(displacement)

    local roadLimRight = math.max(0, roadHalfWidth - roadWidthMargin) -- should be non-negative (zero or positive)
    local newLateralXnorm = clamp(n.lateralXnorm + displacement,
      math.max(n.laneLimLeft + laneWidthMargin, -roadLimRight) + math.max(0, plan.offset or 0) ,
      math.min(n.laneLimRight - laneWidthMargin, roadLimRight) + math.min(0, plan.offset or 0))
    ...
    local function raceCornerLineLiftApply(liftPlan)
      ...
      for i = 2, liftPlan.planCount - 1 do
        ...
        if np and n.vec and np.vec then
          local cInst = math.abs(inCurvature(n.vec, np.vec))
          local kRef = math.max(cInst, math.abs(n.curvature or 0))
          ...
          if t > 0 then
            ...
            if tl > 1e-5 then
              ...
              local roadHalfWidth = n.halfWidth
              local roadLimRight = math.max(0, roadHalfWidth - roadWidthMargin)
              local limL = math.max(n.laneLimLeft + laneWidthMargin, -roadLimRight) + math.max(0, liftPlan.offset or 0)
              local limR = math.min(n.laneLimRight - laneWidthMargin, roadLimRight) + math.min(0, liftPlan.offset or 0)
  ...
  if plan.sideDisp and plan.hitLimit and not (plan.hitLimit * plan.dispLat > 0) then -- side avoidance is pushing me to the opposite limit
    ...
    local newdisp = clamp(targetNode.lateralXnorm + plan.dispLat,
      math.max(targetNode.laneLimLeft + laneWidthMargin, -math.max(0, targetNode.halfWidth - roadWidthMargin)),
      math.min(targetNode.laneLimRight - laneWidthMargin, math.max(0, targetNode.halfWidth - roadWidthMargin)))
  ...
  for i = 2, plan.planCount - 1 do
    ...
    n3vec:setSub2(n1.pos, plan[math.min(plan.planCount, i+2)].pos); n3vec.z = 0
    ...
    local curvatureRateDt = math.min(5 + 0.005 * len * len, 100) * dt
  ...
  ------######## Speed Planning ########-----
  local totalAccel = math.min(aggression, ego.staticFrictionCoef) * g * racingPlannerAccelMult()

  local lastNode = plan[plan.planCount]
  if route.path[lastNode.pathidx+1] or (routeLoop.loopPath and routeLoop.noOfLaps and routeLoop.noOfLaps > 1) then
    if plan.stopSeg <= plan.planCount then
      lastNode.speed = 0
    else
      lastNode.speed = lastNode.manSpeed or math.sqrt(2 * 550 * totalAccel) -- shouldn't this be calculated based on the path length remaining?
    end
  else
    lastNode.speed = lastNode.manSpeed or 0
  end
  lastNode.roadSpeedLimit = plan[plan.planCount-1].roadSpeedLimit
  lastNode.legalSpeed = math.min(lastNode.roadSpeedLimit or math.huge, lastNode.speed)
  ...
  for i = 1, plan.planCount-1 do -- curvature is not defined on the last plan node
    ...
    gT:setSub2(n2.pos, n1.pos); gT:setScaled(gravityDir:dot(gT) / math.max(square(n1.length), 1e-30)) -- gravity vec parallel to road segment: positive when downhill
    local gN = gravityDir:distance(gT) -- gravity component normal to road segment
    n1.acc_max = totalAccel * gN
    n1.speed = math.min(n1.acc_max / math.max(math.abs(n1.curvature), 1e-30), gN * g / math.max(n1.curvatureZ, 1e-30)) -- available centripetal acceleration * radius
  end
  ...
  raceCommit.boostPlanSpeeds(plan)

  --profilerPopEvent('ai_speedProfile')

  plan.targetSpeed = raceCommit.smoothPlanTargetSpeed(raceCommit.applySpeedFloor(plan[1].speed + math.max(0, plan.egoXnormOnSeg) * (plan[2].speed - plan[1].speed)), dt)
  if M.speedMode == 'legal' then
    plan.targetSpeedLegal = plan[1].legalSpeed + math.max(0, plan.egoXnormOnSeg) * (plan[2].legalSpeed - plan[1].legalSpeed)
  else
  ...
  local mergedTS = math.min(geoTS, plan.trafficTargetSpeed)
  -- Racing: recover part of geometric target when traffic overlap caps speed (reduces follow-the-leader in packs).
  if opt.racing then
    local passBlend = (type(parameters.raceTrafficPassBlend) == 'number' and parameters.raceTrafficPassBlend) or 0
    passBlend = clamp(passBlend, 0, 0.88)
    -- Only on clearance-evaluated alt (pmode) or committed alt; keep main plan traffic-safe.
    local evaluatingAlt = pmode == 'left' or pmode == 'right'
    local committedAlt = route and raceAltPlan.normIdx(route.plan_index) ~= 0
    if (evaluatingAlt or committedAlt) and passBlend > 0 and geoTS > mergedTS then
      mergedTS = mergedTS + passBlend * (geoTS - mergedTS)
    end
  end
  ...
  if opt.racing and type(parameters.raceWideLineTrafficBlend) == 'number' and parameters.raceWideLineTrafficBlend > 0 then
    ...
    if refPlan and refPlan.planCount and es > refPlan.planCount - 1 then
      es = math.max(1, refPlan.planCount - 1)
    end
    ...
    if pn and pn1 then
      ...
      local latMag = math.abs((ego.pos - onL):dot(pn.normal))
      local lat0 = (type(parameters.raceWideLineLateralStartM) == 'number' and parameters.raceWideLineLateralStartM) or 0.85
      local lat1 = (type(parameters.raceWideLineLateralEndM) == 'number' and parameters.raceWideLineLateralEndM) or 2.5
      if lat1 <= lat0 then lat1 = lat0 + 0.01 end
      local tWide = clamp((latMag - lat0) / (lat1 - lat0), 0, 1)
...
local function planAhead(route, baseRoute)
  ...
  local minPlanLen
  if M.mode == 'traffic' then
    minPlanLen = getMinPlanLen(20, ego.speed, 0.2 * ego.staticFrictionCoef) -- 0.25 * math.min(aggression, ego.staticFrictionCoef)
  else
  ...
  if not route.pathLength[#route.path] then
    ...
    table.insert(
      route.pathLength,
      mapData.positions[route.path[n+1]]:distance(mapData.positions[route.path[math.max(1, n)]]) + route.pathLength[n]
    )
  end
  ...
  if opt.trajMethod ~= 'springDampers' then
    ...
    for i = 1, plan.planCount-1 do
      ...
      n1.turnDir:setSub2(v1, v2); n1.turnDir:normalize()
      nforce:setScaled2(n1.turnDir, (1-twt.state) * math.max(1 - v1:dot(v2), 0) * parameters.turnForceCoef)
    ...
    for i = 1, plan.planCount-1 do
      ...
      nforce:setScaled2(n1.turnDir, (1-twt.state) * math.max(1 - v1:dot(v2), 0) * parameters.turnForceCoef * stiff)
  ...
  if trafficTableLen > 0 then
    ...
    if opt.racing then
      for _, v in ipairs(traffic.trafficTable) do -- side avoidance loop
        ...
        if ego.speed > 1 and v.vel:dot(ego.dirVec) > 0 and xnormL > 0 and xnormL < 1 and math.abs(v.posFront.z - ego.pos.z) < 4 then
          ...
          local dist = math.min(math.abs(distF), math.abs(distR))
          ...
          local TTC = dist * sign2(rsLat) / (math.abs(rsLat) + 1e-30)
          ...
          if v.dirVec:dot(ego.dirVec) > 0.3 then
            ...
            elseif dist < 0.5 * (ego.width + v.width + 0.2) and TTC > -0.1 then
              if side < 0 then -- v-vehicle is on our left side
                dispLeft = math.max(dispLeft, 0.1)
              elseif side > 0 then -- v-vehicle is on our right side  --(and (xnormF < 0.5 or v.vel:dot(ego.dirVec) > ego.speed))
                dispRight = math.max(dispRight, 0.1)
              end
      ...
      for _, v in ipairs(traffic.trafficTable) do -- side avoidance loop
        ...
        if ego.speed > 1 and v.vel:dot(ego.dirVec) > 0 and xnorm > 0 and xnorm < 1 and math.abs(v.posFront.z - ego.pos.z) < 4 then
          ...
          if xnorm1 > 0 and xnorm1 < 1 and xnorm2 > 0 and xnorm2 < 1 then -- v-vehicle is on our left side
            dispLeft = math.max(dispLeft, linePointFromXnorm(posF, posR, xnorm1):squaredDistance(posR))
          elseif xnorm3 > 0 and xnorm3 < 1 and xnorm4 > 0 and xnorm4 < 1 then -- v-vehicle is on our right side
            dispRight = math.max(dispRight, linePointFromXnorm(posF, posL, xnorm3):squaredDistance(posL))
          end
        end
      end
    end
    ...
    --if fl then
      ...
      --local sideColor = color(clamp(math.sqrt(dispLeft) * 255, 0, 255), 0, clamp(math.sqrt(dispRight) * 255, 0, 255), 160)
      --obj.debugDrawProxy:drawSphere(0.4, midPos + vec3(0,0,2), sideColor)
    --end

    plan.sideDisp = nil
    if dispLeft > 0 or dispRight > 0 then
      local sideDisp = math.sqrt(dispLeft) - math.sqrt(dispRight) -- (sideDisp > 0) means v-vehicle is on our left side and ego should move right
      sideDisp = math.min(dt * parameters.awarenessForceCoef * 10, math.abs(sideDisp)) * sign2(sideDisp) -- limited displacement per frame
      plan.sideDisp = true
      -- maybe needs some smoother to prevent left / right "bouncing"
      local curDist = 0
      local lastPlanIdx = 2
      local targetDist = square(ego.speed) / (2 * g * aggression) + math.max(opt.racing and 100 or 30, ego.speed * 3) -- longer adjustment at higher speeds
    ...
    local egoPathVelInv = 1 / math.abs(egoPathVel + 1e-30)
      ...
      for j = trafficTableLen, 1, -1 do
        ...
        if ego2PlDir > 0 then
          ...
          if v.vel:length() < ego.speed and (v.noproj or opt.racing) then
            velDisp = velDisp * math.min(1,square(square(square(math.max(0,arrivalT/4)))))
          end
          plPosFront = plPosFront + velDisp
          plPosRear = plPosRear + velDisp
        end

        local extVec = 0.5 * math.max(ego.width, plWidth) * nDir
        ...
        if rnorm > 0 and rnorm < 1 and vnorm > 0 and vnorm < 1 then
          ...
          if xnorm > 0 and xnorm < rlen then
            ...
            minSqDist = math.min(minSqDist, v1:squaredDistance(plPosFront))
          end

          xnorm = plPosRear:xnormOnLine(n1ext, n2ext) * rlen
          if xnorm > 0 and xnorm < rlen then
            v1:setScaled2(nDir, xnorm); v1:setAdd(n1ext)
            minSqDist = math.min(minSqDist, v1:squaredDistance(plPosRear))
          end
        ...
        local limWidth = v.targetType == 'follow' and 2 * math.max(n1.radiusOrig, n2.radiusOrig) or plWidth
        ...
        if minSqDist < square((ego.width + limWidth) * 0.8) then
          ...
          if vehicleIsStopped then
            ...
            if distToParked < 25 then
              openPlanLanesIdx = math.max(openPlanLanesIdx, i)
            end
          end
          ...
          if not newOvertake then
            if plan.stopSeg == math.huge and v.targetType ~= 'follow' then -- apply side forces to avoid vehicles
              ...
              if v.sideDir == side1 and inMultipleLanes then -- calculate force coef only if the avoidance side matches the initial value
                ...
                local forceCoef = trafficStates.side.side *
                                  ...
                                  math.max(0, ego.speed - velProjOnSeg, -sign(nDir:dot(v.dirVec)) * trafficStates.side.cTimer) /
                                  ((1 + minSqDist) * (1 + distanceT * math.min(0.1, 1 / (2 * math.max(0, egoPathVel - v.vel:dot(nDir)) + 1e-30))))
            ...
            if plan.stopSeg == math.huge and v.targetType ~= 'follow' and not v.overtaken then
              ...
              if inMultipleLanes then
                ...
                local factor2 = math.max(0,-sign(nDir:dot(v.dirVec)) * trafficStates.side.cTimer)
                for ii = 2, plan.planCount-1 do
                  ...
                  if v.sideDir == sign(distY) then
                    local forceCoef = sign(distY)*math.max(0, ((n1i.radiusOrig) - math.abs(distY))*math.exp(-0.001*square(Deltax)))*(1*parameters.awarenessForceCoef*dt)
                    forceCoef = forceCoef*math.max(factor1, factor2)*trafficStates.side.side
          ...
          if M.mode ~= 'flee' and M.mode ~= 'random' and (M.mode ~= 'manual' or (n1.laneLimRight - n1.laneLimLeft) <= ego.width + plWidth) then
            ...
            if i == 2 and minSqDist < square((ego.width + limWidth) * 0.6) and ego2PlDir > 0 and v.vel:dot(ego.rightVec) * ego2PlVec:dot(ego.rightVec) < 0 then
              n1.trafficSqVel = math.max(0, n1.trafficSqVel - math.abs(1 - v.vel:dot(ego.dirVec)) * (v.vel:length()))
            end
          end
        end
      end
    ...
    trafficStates.block.block = math.max(trafficMinSpeedSq, ego.speed * ego.speed) < 1 and (minTrafficDir < -0.7 or trafficStates.intersection.block)
  ...
  if route.laneChanges[1] and math.floor(plan[2].rangeLaneCount + 0.5) > 1 and route.laneChanges[1].side ~= 0 then
    ...
    if plan[plan.planCount].pathidx > exitNodeIdx then
      ...
      distToExit = plan.planLen + math.max(0, route.pathLength[route.laneChanges[1].pathIdx] - route.pathLength[plan[plan.planCount].pathidx])
    end

    if (distToExit < math.min(600, math.max(15, ego.speed * ego.speed * 0.7)) or route.laneChanges[1].commit) then --and not plan.noExit
      ...
      if not plan.noExit or route.laneChanges[1].commit then
        ...
        for i = 3, planExitIdx do
          ...
          if side < 0 then
            ...
            n.laneLimRight = math.max(n.laneLimLeft + ego.width, math.min(n.laneLimRight, n.lateralXnorm + laneHalfWidth))
            forces[i]:setAdd(planDist * sideForceCoeff * square(math.min(1, 0.25 * math.abs(n.lateralXnorm - n.laneLimLeft))) * n.normal)
          else
            n.laneLimRight = (2 * n.rangeRight - 1) * roadHalfWidth -- open right lane limit to right range limit
            n.laneLimLeft = math.min(n.laneLimRight - ego.width, math.max(n.laneLimLeft, n.lateralXnorm - laneHalfWidth))
            forces[i]:setAdd(planDist * sideForceCoeff * square(math.min(1, 0.25 * math.abs(n.laneLimRight - n.lateralXnorm))) * n.normal)
          end
        end
        for i = planExitIdx + 1, plan.planCount do
          planDist = math.max(0, planDist - plan[i-1].length)
        ...
        --------- Plan cleaning -----------
        local clearIdx = math.max(exitNodeIdx-1, plan[2].pathidx)
        ...
        for i = #route.path, math.min(#route.path, exitNodeIdx) + 1, -1  do
  ...
  for i = 2, plan.planCount-skipLast do
    ...
    if opt.driveInLaneFlag and not n.lanesOpen then
      ...
      local dispToLeftRange = math.min(1, math.max(0, b - (n.lateralXnorm - rangeLeft)) / b)
      local dispToRightRange = math.min(1, math.max(0, b - (rangeRight - n.lateralXnorm)) / b)
      forceToLaneCenter = 0.15 * (dispToLeftRange - dispToRightRange)
    end

    -- Calculate node displacement: avoids switching displacement direction (sign) in concecutive frames
    local displacement = forceToLaneCenter + math.max(math.min(n.normal:dot(forces[i]), forceMagLim), -forceMagLim)
    displacement = displacement * math.max(math.min((displacement * n.dispDir) * math.huge, 1), 0) -- second term returns 0 if (displacement * n.dispDir) is negative, 1 otherwise.
    n.dispDir = sign(displacement)

    local roadLimRight = math.max(0, roadHalfWidth - roadWidthMargin) -- should be non-negative (zero or positive)
    local limLow = math.max(n.laneLimLeft + laneWidthMargin, -roadLimRight)
    local limHigh = math.min(n.laneLimRight - laneWidthMargin, roadLimRight)
    local limAvg = (limLow + limHigh) * 0.5 -- guard against clamp limit crossing

    local newLateralXnorm = clamp(n.lateralXnorm + displacement, math.min(limLow, limAvg), math.max(limHigh, limAvg))
  ...
  if lastNodeLatXnorm then
    ...
    plan[plan.planCount].laneLimLeft = math.max(rangeLeft, math.min(latXnorm, lastNodeLimLeft + disp))
    plan[plan.planCount].laneLimRight = math.min(rangeRight, math.max(latXnorm, lastNodeLimRight + disp))
  end
  ...
  if parameters.planErrorSmoothing and plan.targetPos and plan.targetSeg and plan.planCount > plan.targetSeg and twt.state == 0 then
    ...
    dTotal = math.max(1, dTotal + plan.targetPos:distance(plan[plan.targetSeg].pos))
  ...
  if not opt.racing then
    ...
    for i = 2, plan.planCount - 1 do
      ...
      v3z:set(n1.length + (n2.length or 0), 0, plan[math.min(plan.planCount, i + 2)].posOrig.z - n1.posOrig.z)

      local curvatureZ = math.min(inCurvature(v1z, v2z), inCurvature(v1z, v3z))
      --dump(i,'curvature', curvature, 'Zcurv', curvatureZ)

      -- calculate curvature temporal smoothing parameter (fast reacting, time dependent)
      local curvatureRateDt = math.min(25 + 0.000045 * len * len * len * len, 1000) * dt
  ...
  if speedProfileMode then
    ...
    for i = plan.planCount-1, 1, -1 do
      ...
      local curvature = math.max(math.abs(n1.curvature), 1e-5)
      ...
      if plan.stopSeg <= i then
        ...
        n1SpeedSq = math.min(n1.trafficSqVel, turnSpeedSq * math.sin(math.min(math.asin(math.min(1, square(n2.speed) / turnSpeedSq)) + 2 * curvature * n1.length, math.pi * 0.5)))
      end

      n1.speed = n1.manSpeed or
                  (M.speedMode == 'limit' and M.routeSpeed and math.min(M.routeSpeed, math.sqrt(n1SpeedSq))) or
                  (M.speedMode == 'set' and M.routeSpeed) or
                  math.sqrt(n1SpeedSq)
      ...
      if M.speedMode == 'legal' then
        ...
        if plan.stopSeg <= i then
          ...
          local n1LegalSpeedSq = math.min(n1.trafficSqVel, turnSpeedSq * math.sin(math.min(math.asin(math.min(1, square(n2.legalSpeed) / turnSpeedSq)) + 2 * curvature * n1.length, math.pi * 0.5)))
          if n1.roadSpeedLimit then
            n1.legalSpeed = math.min(math.sqrt(n1LegalSpeedSq), n1.roadSpeedLimit * (1 + aggression * 2 - 0.6))
          else
            n1.legalSpeed = math.sqrt(n1LegalSpeedSq)
          end
        end
      end
  ...
  plan.targetSpeed = raceCommit.smoothPlanTargetSpeed(plan[1].speed + math.max(0, plan.egoXnormOnSeg) * (plan[2].speed - plan[1].speed), dt)
  if M.speedMode == 'legal' then
    plan.targetSpeedLegal = plan[1].legalSpeed + math.max(0, plan.egoXnormOnSeg) * (plan[2].legalSpeed - plan[1].legalSpeed)
  else
    plan.targetSpeedLegal = math.huge
  ...
  --plan.targetSpeed = math.min(plan.targetSpeed, plan.trafficTargetSpeed)
...
local function resetMapAndRoute()
  ...
  signalsData = nil
  currentRoute = nil
  routeLoop.loopPath = nil
  routeLoop.noOfLaps = nil
...
local function setScriptedPath(arg)
  ...
  setParameters({
    ...
    staticFrictionCoefMult = math.max(0.95, arg.staticFrictionCoefMult or 0.95),
    lookAheadKv = math.max(0.1, arg.lookAheadKv or parameters.lookAheadKv),
  ...
  routeLoop.noOfLaps = arg.noOfLaps
  routeLoop.loopPath = arg.loopPath
...
local function fleePlan()
  if opt.aggressionMode == 'rubberBand' then
    setAggressionInternal(math.max(0.3, 0.95 - 0.0015 * player.pos:distance(ego.pos)))
  end
  ...
  if not currentRoute or internalState.changePlanTimer == 0 or currentRoute.plan.reRoute then
    ...
    if route and route.plan then
      ...
      if not currentRoute or internalState.changePlanTimer > 0 or tempPlan.targetSpeed >= math.min(ego.speed, currentRoute.plan.targetSpeed) and targetsCompatible(currentRoute, route) then
        currentRoute = route
        internalState.changePlanTimer = math.max(1, internalState.changePlanTimer)
        return
...
local function chasePlan()
  ...
  if dist1 > math.max(radii[wp1], radii[wp2]) + ego.width and dist2 > math.max(radii[plwp1], radii[plwp2]) + obj:getObjectInitialWidth(player.id) then
    internalState.road = 'offroad'
    return
  end
  ...
  local nearDist = math.max(ego.length + 8, internalState.chaseData.playerStoppedTimer) -- larger if player stopped for longer (anti softlock)
  local isAtPlayerSeg = (wp1 == playerNode or wp2 == playerNode)

  if opt.aggressionMode == 'rubberBand' then
    if M.mode == 'follow' then
      setAggressionInternal(math.min(0.75, 0.3 + 0.0025 * egoPlDist))
    else
      setAggressionInternal(math.min(0.95, 0.8 + 0.0015 * egoPlDist))
    end
  end
  ...
  if internalState.chaseData.playerStoppedTimer > 5 and egoPlDist < math.max(nearDist, square(ego.speed) / (2 * g * aggression)) then -- within braking distance to player
  ...
  if currentRoute then
    ...
    local playerIncoming = playerSpeed >= 3 and playerNode == wp1 and egoPlDist < math.max(ego.speed, playerSpeed) and playerVel:dot(planVec) < 0 -- player is driving towards or past ego on the segment
    ...
    if not playerNodeInPath and not internalState.chaseData.driveAhead and (ego.speed < 3 or ego.dirVec:dot(player.pos - ego.pos) > 0) then -- prevents ego from cancelling its current route if it should slow down to turn around
      ...
      if route and route.plan then --and tempPlan.targetSpeed >= math.min(ego.speed, curPlan.targetSpeed) and (tempPlan.targetPos-curPlan.targetPos):dot(ego.dirVec) >= 0 then
        currentRoute = route
      end
    end
    ...
    if M.mode == 'chase' then
      ...
      local crashSpeed = (type(parameters.chaseCrashSpeed) == 'number' and parameters.chaseCrashSpeed) or 10
      if egoPlDist < math.max(brakeDist, nearDist) and ego.dirVec:dot(egoPosRear - player.pos) < 0 then
        targetSpeed = math.max(crashSpeed, relSpeed + crashSpeed)
    ...
    if not internalState.chaseData.driveAhead then
      ...
      elseif playerBehind then -- match the player speed based on distance
        targetSpeed = clamp(playerSpeed * (1 - egoPlDist / 120), 5, math.max(5, curPlan[2].speed))
      end
    end
...
local function trafficActions()
  ...
  if parameters.enableElectrics and trafficStates.action.hornTimer == 0 then
    electrics.horn(true)
    trafficStates.action.hornTimerLimit = math.max(0.1, math.random())
  end
  ...
  for plID, v in pairs(mapmgr.getObjects()) do
    if plID ~= objectId and v.states then
      if v.states.lightbar == 2 or (v.states.lightbar == 1 and v.vel:squaredLength() >= 100) then
        local posFront = obj:getObjectFrontPosition(plID)
        minSirenSqDist = math.min(minSirenSqDist, posFront:squaredDistance(ego.pos))
          ...
          for otherID, v in pairs(mapmgr.getObjects()) do
            if otherID ~= objectId and v.pos and otherID ~= trafficStates.action.nearestPoliceId and v.dirVec and v.dirVec:dot(ego.dirVec) < 0 then
              ...
              if distance < 225 then -- within 15 meters
                ...
                if math.abs(forwardDist) < 10 and math.abs(lateralDist) < 6 then -- vehicle is alongside within 8m forward/back and 6m lateral
  ...
  if pullOver and not trafficStates.action.forcedStop and ego.speed >= 3 then
    local brakeDist = square(ego.speed) / (2 * g * aggression)
    local dist = math.max(10, brakeDist)
    ...
    laneChange(plan, math.max(10, dist - 20), disp * side)
  ...
  if trafficStates.action.forcedStop and ego.speed < math.min(plan.targetSpeed or 0, 1) then -- instant plan stop
    setStopPoint(plan, 0)
  end
  ...
  if not tSi.node then
    ...
    for i = tSi.startIdx, #path - 1 do -- TODO: would searching up to math.min(#path-1, plan[plan.planCount].pathIdx) work to distribute the load over more frames work?
      ...
      if nid1 and nid2 then
        ...
        if tableSize(mapData.graph[nid1]) > 2 and not tSi.node and tSi.startIdx > 0 then
          ...
          local giveWay = math.abs(nDir:dot(linkDir)) < 0.707
          ...
          if giveWay then
            ...
            if not (fourthNode and nContinuation ~= nid2 and dotDir > 0) then
              ...
              local pos = n1Pos - nDir * (math.max(3, mapData.radius[nid1]) + 2) --needs at least + 3
      ...
      if tSi.node then
        ...
        tSi.timer = 0
        if tSi.turnDir then
          if math.abs(tSi.dir:dot(tSi.turnDir)) < 0.707 then
  ...
  if tSi.node and not trafficStates.action.forcedStop then
    ...
    local brakeDist = square(ego.speed) / (2 * g * ego.staticFrictionCoef * math.min(1, aggression * 1.3))
            ...
            for vId, v in pairs(mapmgr.getObjects()) do
              if vId ~= objectId then
                ...
                if not isVehicleStopped(v) and v.dirVec:dot(vPosF - (ego.pos + distToJcenter * ego.dirVec)) < 0 then
                  checkDirRad = v.dirVec:dot(tSi.dir) < 0.707 and egoCenterPos:squaredDistance(vPosF) < square(math.max(math.min(60, math.max(40, v.vel:squaredLength() / (g * ego.staticFrictionCoef))), v.vel:length()*4))
                end
                ...
                if check then
                  ...
                  local ks = 0.9
                  tSi.timer = tSi.timer - (kr + math.min(v.vel:squaredLength(),1)*(ks - kr)/1)*dt
                  tSi.timer = math.max(tSi.timer, 0)
                  break
                end
              end
            end
      ...
      if tSi.proximity then
        ...
        if distSq > 400 then -- assumes that vehicle has cleared the intersection (20 m away from the signal point)
          ...
          if plan[1].pathidx < plan[2].pathidx then -- plan[1] node is on a different path index than the node ahead of it
            ...
            startIdx = math.max(startIdx, plan[1].pathidx)
          else
            startIdx = math.max(startIdx, plan[1].pathidx-1)
          end
          table.clear(tSi)
          tSi.timer = 0
          tSi.turn = 0
    ...
    if parameters.enableElectrics and tSi.turnNode and ego.pos:squaredDistance(mapData.positions[tSi.turnNode]) < square(math.max(50, brakeDist * 1.2)) then -- approaching intersection
...
local function trafficPlan()
  ...
  if currentRoute and currentRoute.path[1] and not currentRoute.plan.reRoute and trafficStates.block.timer <= trafficStates.block.timerLimit then --and not currentRoute.plan.noExit
    ...
    if path and path[1] then
      ...
      if route and route.plan then
        trafficStates.block.timerLimit = math.max(1, parameters.trafficWaitTime * 2)
        ...
        elseif not currentRoute then
          currentRoute = route
          return
        elseif route.plan.targetSpeed >= math.min(currentRoute.plan.targetSpeed, ego.speed) and targetsCompatible(currentRoute, route) then
          currentRoute = route
          return
        end
      end
...
local function targetFollowControl(targetSpeed, distLim) -- throttle and brake control for when driving directly to player
  ...
  if not targetSpeed then
    ...
    targetSpeed = math.sqrt(math.max(0, math.abs(plSpeedFromego) * plSpeedFromego + 2 * g * math.min(aggression, ego.staticFrictionCoef) * ego2PlDist))

    -- WIP
    -- throttleTargetSpeed = math.sqrt(math.max(0, math.abs(plSpeedFromego) * plSpeedFromego + 2 * g * math.min(aggression, ego.staticFrictionCoef) * (minHit - (distLim or 3) - ego.speed * 1)))
    -- brakeTargetSpeed = math.sqrt(math.max(0, math.abs(plSpeedFromego) * plSpeedFromego + 2 * g * math.min(aggression * 1.5, ego.staticFrictionCoef) * (minHit - (distLim or 3))))
    -- targetPos = catmullRomChordal(ego.pos-ego.dirVec, ego.pos, plC-plX, plC-plX+plX:normalized(), math.min(1, math.max(ego.speed * parameters.lookAheadKv, 4.5) / ego.pos:distance(plC - plX)))
    ...
    if nodeCollisionIdx then
      ...
      for i = 1, changeSetCount, 3 do
        if changeSet[i+2] < 0 then
          ...
          changeSetDict[table.concat(edgeTab)] = true
        end
      end
      ...
      for i = math.max(currentRoute.plan[2].pathidx, nodeCollisionIdx - 1), #path-1 do
        local nodeA, nodeB = path[i], path[i+1]
        edgeTab[1] = nodeA < nodeB and nodeA or nodeB
        edgeTab[3] = nodeA == edgeTab[1] and nodeB or nodeA
        if changeSetDict[table.concat(edgeTab)] then
          edgeCollisionIdx = i
...
local function modeAdjustments()
    ...
    if opt.aggressionMode == 'rubberBand' then
      ...
      if player ~= nil then
        if (ego.pos - player.pos):dot(ego.dirVec) > 0 then
          setAggressionInternal(math.max(math.min(0.1 + math.max((150 - player.pos:distance(ego.pos)) / 150, 0), M.extAggression), 0.5))
        else
          setAggressionInternal()
        end
      end
    ...
    if opt.racing then
      ...
      if currentRoute then
        ...
        if mainPlan.distancesV < 10000 then
          if raceAltPlan.runAltPlans then
          ...
          local cost_time = mainPlan.planLen/math.max(mainPlan.targetSpeed, 0.1)
          local speed_margin = (type(parameters.raceAltPlanSpeedMargin) == 'number' and parameters.raceAltPlanSpeedMargin) or 2
          local time_margin = (type(parameters.raceAltPlanTimeMargin) == 'number' and parameters.raceAltPlanTimeMargin) or 1
          ...
          do
            ...
            local throttle_check = (lastCommand.throttle > 0 or mainPlan.targetSpeed == mainPlan.trafficTargetSpeed) and clamp((kplan.targetSpeed - ego.speed) * parameters.throttleKp, 0, 1) > math.min(current_throttle + delta_throttle, 1)
            ...
            local kcurvature = 2 * math.abs(d:dot(n)) * dSqLengthInv
            local axSq = square(0.5 * math.max(egoSqspeed - square(kplan[kplan.targetSeg].speed), 0)) * dSqLengthInv
          ...
          -- Main line blocked: prefer offset plan with more corridor (distance along plan / traffic cap), even if not faster.
          if not plan_index then
            local distMax = (type(parameters.raceBlockedAheadDistMax) == 'number' and parameters.raceBlockedAheadDistMax) or 28
            local minGain = (type(parameters.raceClearancePassMinGainM) == 'number' and parameters.raceClearancePassMinGainM) or 0.85
            local trEps = (type(parameters.raceClearancePassTrafficEps) == 'number' and parameters.raceClearancePassTrafficEps) or 0.65
            local blockedMain = mainPlan.distances < math.huge and mainPlan.distances < distMax
            if blockedMain then
              local bestIdx, bestScore = nil, -math.huge
              local function tryClearancePass(sideIdx, kplan, sideGate)
                if not kplan or not kplan[2] then return end
                if sideGate <= 0 then return end
                local dGain = kplan.distances - mainPlan.distances
                local tGain = kplan.trafficTargetSpeed - mainPlan.trafficTargetSpeed
                if dGain < minGain and tGain < trEps then return end
                d:set((sideIdx == 1 and -0.1 or 0.1) * push3(mainPlan[mainPlan.targetSeg].normal) + rearEgoPos2TargetPos)
                local dSqLengthInv = 1 / (d:squaredLength() + 1e-30)
                local kcurvature = 2 * math.abs(d:dot(n)) * dSqLengthInv
                local axSq = square(0.5 * math.max(egoSqspeed - square(kplan[kplan.targetSeg].speed), 0)) * dSqLengthInv
                if square(mainPlan[1].acc_max) - axSq <= square(egoSqspeed * kcurvature) then return end
                local score = dGain * 2.2 + tGain * 1.1 + kplan.targetSpeed * 0.08
                if score > bestScore then
                  bestScore = score
                  bestIdx = sideIdx
                end
              end
              tryClearancePass(1, currentRoute.planL, ((mainPlan.dispLat and mainPlan.dispLat ~= 0) and -mainPlan.dispLat or 1))
              tryClearancePass(2, currentRoute.planR, ((mainPlan.dispLat and mainPlan.dispLat ~= 0) and mainPlan.dispLat or 1))
              plan_index = bestIdx
            end
          end

          -- Race lateral hysteresis (cand = raw plan_index this frame, applied = committed plan_index).
          -- - Center + raw side: _raceAltPlanHystPending is that side; time accrues while raw keeps picking it; cand==0 clears pending (vanished side cannot promote).
          -- - Lateral + raw center (cand==0): clear stale side-pending only; accrue time so hold below can release and allow center to commit.
          -- - Hold: while cand ~= applied and time < raceAltPlan.hystTime, keep plan_index on applied (or nil if applied is center).
          do
            local cand = raceAltPlan.normIdx(plan_index)
            local applied = raceAltPlan.normIdx(currentRoute.plan_index)
            local hystDt = dt or 0
            local pend = currentRoute._raceAltPlanHystPending

            if cand == applied and applied ~= 0 then
              currentRoute._raceAltPlanHystPending = nil
              currentRoute._raceAltPlanHystTime = 0
            elseif cand == 0 and applied == 0 then
              currentRoute._raceAltPlanHystPending = nil
              currentRoute._raceAltPlanHystTime = 0
            elseif pend and cand ~= 0 and cand ~= pend then
              currentRoute._raceAltPlanHystPending = cand
              currentRoute._raceAltPlanHystTime = hystDt
            elseif cand ~= 0 and cand == pend then
              currentRoute._raceAltPlanHystTime = (currentRoute._raceAltPlanHystTime or 0) + hystDt
            elseif cand == 0 and applied ~= 0 then
              currentRoute._raceAltPlanHystPending = nil
              currentRoute._raceAltPlanHystTime = (currentRoute._raceAltPlanHystTime or 0) + hystDt
            elseif cand ~= 0 and cand ~= applied then
              currentRoute._raceAltPlanHystPending = cand
              currentRoute._raceAltPlanHystTime = hystDt
            end

            pend = currentRoute._raceAltPlanHystPending
            local hystTime = currentRoute._raceAltPlanHystTime or 0
            if pend and hystTime >= raceAltPlan.hystTime and applied == 0 then
              plan_index = pend
              cand = raceAltPlan.normIdx(plan_index)
            end

            if cand ~= applied and hystTime < raceAltPlan.hystTime then
              if applied == 0 then
                plan_index = nil
              elseif applied == 1 then
                plan_index = 1
              else
                plan_index = 2
              end
          ...
          if plan_index then
            ...
            -- Center hold building a side candidate: keep pending/time. Otherwise clear (lateral→center timer is not stored on nil commit; hold keeps plan_index non-nil until release).
            if not (currentRoute._raceAltPlanHystPending and (currentRoute._raceAltPlanHystTime or 0) < raceAltPlan.hystTime) then
              currentRoute._raceAltPlanHystPending = nil
              currentRoute._raceAltPlanHystTime = 0
            end
          end
  ...
  ------------------ DEMO DERBY MODE ------------------
  elseif M.mode == 'demoderby' then
    updatePlayerData()
    if player then
      internalState.road = 'offroad'
      opt.avoidCars = 'off'
      
      -- ── checkHitTarget: detect collisions and trigger crash.manoeuvre ──
      -- Active contact starts a persistent escape manoeuvre and requests a new target.
      internalState.derby.checkCollision()
      
      local throttle, brake = targetFollowControl(math.huge)
      
      -- Executing the core physics and driving logic replicated from demo_green_derby
      driveToTargetDerby(player.pos, throttle, brake)
      return true
    else
      driveCar(0, 0, 0, 1)
      return true
  ...
  ------------------ DEMO DERBY RECOVERY MODE ------------------
  elseif M.mode == 'demoderby_recovery' then
    updatePlayerData()
    if player then
      internalState.road = 'offroad'
      opt.avoidCars = 'on' -- Avoid crashing so they don't get stuck out of bounds!
      local throttle, brake = targetFollowControl(math.huge)
      driveToTargetDerby(player.pos, throttle, brake)
      return true
    else
  ...
  ------------------ DEMO DERBY REVERSE MODE ------------------
  elseif M.mode == 'demoderby_reverse' then
    updatePlayerData()
    if player then
      internalState.road = 'offroad'
      opt.avoidCars = 'off'
      ...
      internalState.derby.checkCollision()
      if internalState.derby.recoveryTimer > 0 then
        driveToTargetDerby(player.pos, 1, 0)
        return true
      end
      
      local plego = player.pos - ego.pos
      local dist = plego:length()
      local targetPos = player.pos

      local closingSpd = (ego.vel - player.vel):dot(plego / (dist + 1e-30))
      
      if player.vel:length() > 2 and dist > 10 and dist < 100 then
        local timeToImpact = dist / math.max(1, closingSpd + ego.speed)
        timeToImpact = math.min(timeToImpact, 2.5)
        targetPos = player.pos + player.vel * timeToImpact
      end

      local targetVec = targetPos - ego.pos
      targetVec:normalize()
      local dirAngle = math.asin(ego.rightVec:dot(targetVec))
      local dirTarget = ego.dirVec:dot(targetVec)
      
      local steering = 0
      local brakeAmt = 1  -- brake maps to reverse in arcade mode
      local throttleAmt = 0 
      
      if dirTarget < -0.2 then
        -- Target is behind us! Steer towards it.
        steering = dirAngle * 2 
      else
        -- Target is in front, swing the rear around!
        steering = (dirAngle > 0 and -1 or 1)
      end
      
      -- ensure arcade mode so brake does reverse automatically
      if controller.mainController then
        controller.mainController.setGearboxMode('arcade')
      end

      -- If we are going forward too fast, we brake. If we are stopped or going backwards, we reverse.
      driveCar(math.max(-1, math.min(1, steering)), throttleAmt, brakeAmt, 0)
      return true
    else
      driveCar(0, 0, 0, 1)
      return true
  ...
  elseif M.mode == 'stop' then
    if currentRoute then
      planAhead(currentRoute)
      local targetSpeed = math.max(0, ego.speed - math.sqrt(math.max(0, square(ego.staticFrictionCoef * g) - square(sensors.gx2))) * dt) -- TODO: check this calculation, i don't think it does what you think it does
    ...
    if aggressionMode == 'rubberBand' then
      ...
      if player ~= nil then
        if (ai.pos - player.pos):dot(ai.dirVec) > 0 then
          setAggressionInternal(math.max(math.min(0.1 + math.max((150 - player.pos:distance(ai.pos)) / 150, 0), M.extAggression), 0.5))
        else
          setAggressionInternal()
        end
      end
...
local function updateGFX(dtGFX)
  ...
  ego.pos.z = math.max(ego.pos.z - 1, obj:getSurfaceHeightBelow(ego.pos))
  ...
  local frictionMult = parameters.staticFrictionCoefMult
  if opt.racing and parameters.raceCommitCorners ~= false then
    local rfm = parameters.raceFrictionMult
    frictionMult = (type(rfm) == 'number' and rfm > 0) and rfm or math.max(frictionMult or 1, 1.06)
  end
  ego.staticFrictionCoef = frictionMult * obj:getStaticFrictionCoef() -- depends on ground model, tire and tire load

  local derbyMode = M.mode == 'demoderby' or M.mode == 'demoderby_reverse' or M.mode == 'demoderby_recovery'
  if derbyMode then
    internalState.derby.recoveryCooldown = math.max(0, internalState.derby.recoveryCooldown - dt)
  end
  local commandedEffort = derbyMode and math.max(lastCommand.throttle, lastCommand.brake) or lastCommand.throttle
  local effortThreshold = derbyMode and 0.25 or 0.5
  if commandedEffort > effortThreshold and ego.speed < 1 then
    internalState.egoCannotMoveTime = internalState.egoCannotMoveTime + dt
  else
    internalState.egoCannotMoveTime = 0
  end

  if derbyMode and internalState.egoCannotMoveTime > 1.1 then
    internalState.derby.beginRecovery('stuck')
  elseif derbyMode and ego.speed > 3 and internalState.derby.recoveryTimer <= 0 and internalState.derby.recoveryCooldown <= 0 then
    internalState.derby.recoveryAttempts = 0
  end
  ...
  if ego.speed < 3 then
    ...
    trafficStates.side.cTimer = math.max(0, trafficStates.side.cTimer - dt)
  ...
  if recover.recoverOnCrash then
    if ego.speed < 3 then
      ...
      recover.recoverTimer = math.max(0, recover.recoverTimer - 5 * dt)
    end
  ...
  internalState.changePlanTimer = math.max(0, internalState.changePlanTimer - dt)
  ...
  if currentRoute then
    local plan = currentRoute.plan
    local targetPos = plan and plan.targetPos
    local egoSeg = plan and plan.egoSeg

    -- planAhead can bail after resetting to a one-node stub (planCount 1, no egoSeg)
    -- when the vehicle is off-graph (auction block). Stock ai.lua indexes plan[egoSeg]
    -- and fatals. Drop the dead route; do not hold brakes forever (that left cars
    -- "started but stuck" after bidding when exit used road AI from the stall).
    if not plan or not egoSeg or not plan[egoSeg] then
      currentRoute = nil
      return
    end

    -- cleanup path if it has gotten too long
    if not opt.racing and not routeLoop.loopPath and plan[egoSeg].pathidx >= 10 and currentRoute.path[20]  then
      local path = currentRoute.path
    ...
    local targetSpeed = raceCommit.applyCornerExitPush(plan.targetSpeed, plan, dt)
    ...
    if not plan[egoSeg+2] and ((targetSpeed < lowTargetSpeedVal and ego.speed < 0.15) or (targetPos - ego.pos):dot(ego.dirVec) < 0) then
      ...
      if ego.speed < 0.15 then
        ...
        driveCar(0, 0, math.max(0.3, lastCommand.brake), 0)
      end
      return
    end
    ...
    local lowSpeedDif = math.min(speedDif - clamp((ego.speed - 2) * 0.5, 0, 1), legalSpeedDif) * throttleKpEffective
    ...
    if opt.racing and speedDif > 0.15 and throttle > 0 then throttle = 1 end

    local brakeLimLow = sign(math.max(0, lowTargSpeedConstBrake)) * 0.5

    local brake = sign(math.max(0, (electrics.values.smoothShiftLogicAV or 0) - 3))
    brake = clamp(-speedDif, brakeLimLow, 1) * brake -- arcade autobrake comes in at |smoothShiftLogicAV| < 5
    if opt.racing and brake > 0 then
      local bg = (type(parameters.raceBrakeGain) == 'number' and parameters.raceBrakeGain) or 0.78
      brake = math.min(1, brake * clamp(bg, 0.35, 1.25))
    end

    if brake > 0 and math.abs(speedDif) < 0.5 and lastCommand.throttle == 0 and lastCommand.brake == 0 then
...
local function debugDraw(focusPos)
  ...
  if currentRoute then
    ...
    elseif M.debugMode == 'route' then
      ...
      local count = math.min(#visDebug.routeRec, maxCount)
      ...
      if visDebug.candidatePaths and visDebug.candidatePaths[1] then
        ...
        for i = 1, #visDebug.candidatePaths do
          ...
          local txt = thisPathLastNode.." -> "..string.format("%0.4f", thisScore)
    ...
    elseif M.debugMode == 'speeds' then
      ...
      for k = 1, 2 do -- left and right plan
        ...
        if altPlan and plan.buildN then
          ...
          for j = 1, altPlan.planCount do
            ...
            debugDrawer:drawText(point.pos + vec3(0,0,speed*0.2), color(0, 0, 0, 255), string.format("%2.0f", speed*3.6).." kph")
            if j > 1 then
      ...
      local maxCount = 175
      local count = math.min(#visDebug.trajecRec, maxCount)
      ...
      for i = 1, count-1 do
        ...
        debugDrawer:drawSquarePrism(visDebug.trajecRec[1 + (last + i - 1) % count][1], n[1], tmpVec1, tmpVec1, color(255 * math.sqrt(math.abs(n[4])), 255 * math.sqrt(n[5]), 0, 100))
      end
      ...
      for i = 1, count-1 do
        ...
        if focusPos:squaredDistance(v[1]) < visDebug.labelRenderDistance * visDebug.labelRenderDistance then
          ...
          debugDrawer:drawText(v[1] + tmpVec1 + zOffSet, yellow, string.format("%2.0f", v[2]*3.6).." kph")

          tmpVec1:set(0, 0, v[3] * 0.2)
          debugDrawer:drawText(v[1] + tmpVec1 + zOffSet, blue, string.format("%2.0f", v[3]*3.6).." kph")
        end
        prevEntry = v
      end
      ...
      if plan[1] then
        ...
        for i = 1, #plan do
          ...
          if M.speedMode == 'legal' then
            ...
            debugDrawer:drawText(p1_, black, string.format("%2.0f", legalSpeed*3.6).." kph")
        ...
        for i = math.max(1, plan[1].pathidx-2), #currentRoute.path-2 do
    ...
    elseif M.debugMode == 'trajectory' then
      ...
      --     debugDrawer:drawCylinder(p, p + vec3(0, 0, scaledV), 0.06, color(math.abs(math.min(sign(v),0))*255,math.max(sign(v),0)*255,0,200))
      --     debugDrawer:drawText(p + vec3(0, 0, scaledV), color(0,0,0,255), string.format("%5.4e", v))
      ...
      for i = 1, count-1 do
        local n = visDebug.trajecRec[1+(last+i)%count]
        debugDrawer:drawSquarePrism(visDebug.trajecRec[1+(last+i-1)%count][1], n[1], tmpVec, tmpVec, color(255 * math.sqrt(math.abs(n[3])), 255 * math.sqrt(n[2]), 0, 100))
      end
    elseif false and M.debugMode == 'rays' then
      ...
      for i = twt.idx, rounds do --the ray cast loop: each iteration scans one corner and one side
        ...
        if twt.rayMins[i] > rayLen then -- odd index is corners
          ...
          helperVec:setLerp(twt.dirTable[twt.RRT[j-1]], twt.dirTable[twt.RRT[j]], twt.minRayCoefs[i]) -- LERP corner direction
          helperVec:normalize()
          twt.rayMins[i] = castRay(tmpVec, helperVec, math.min(2 * twt.rayMins[i], rayDist))
        end
      end
  ...
  * examples:
  ...
  if arg.script then
    ...
    if dir then
      ...
      M.mode = 'manual'
      stateChanged()
      routeLoop.noOfLaps = math.max(arg.noOfLaps or 1, 1)
      local pathMap = require('graphpath').newGraphpath() -- create a dummy graph

      local scrCount = #arg.script
      if routeLoop.noOfLaps > 1 and arg.script[scrCount].x == arg.script[1].x and arg.script[scrCount].y == arg.script[1].y and arg.script[scrCount].z == arg.script[1].z then
        routeLoop.loopPath = true
      end
      ...
      for i = 1, scrCount - (routeLoop.loopPath and 1 or 0) do -- avoid adding the last point if the scripts loops
      ...
      scriptData.noOfLaps = routeLoop.noOfLaps
      scriptData.loopPath = routeLoop.loopPath
    end
  else
    ...
    if routeLoop.noOfLaps > 1 and ((wpList and wpList[2] and wpList[1] == wpList[#wpList]) or (manualPath and manualPath[2] and manualPath[1] == manualPath[#manualPath])) then
      routeLoop.loopPath = true
    end
...
M.beginDerbyRecovery = internalState.derby.beginRecovery
M.getDerbyRecoveryState = function()
  return {
    timer = internalState.derby.recoveryTimer,
    cooldown = internalState.derby.recoveryCooldown,
    attempts = internalState.derby.recoveryAttempts,
    direction = internalState.derby.recoveryDirection,
    reason = internalState.derby.reason,
  }
end
