...
local function updateFixedStep(dt)
  ...
  local speedCoef = movementSpeedCoef
  local moveInput = walkVector
  local worldSpaceMove = false
  if externalMove then
    moveInput = vec3(externalMove.x or 0, externalMove.y or 0, 0)
    worldSpaceMove = externalMove.worldSpace == true
    if externalMove.speedCoef ~= nil then
      speedCoef = externalMove.speedCoef
    end
  end
  local desiredMovementSpeed = linearScale(speedCoef, 0, 1, movementSpeedNormal, movementSpeedSprint) * frozenCoef --0 movement speed when locked
  local guardedWalkVector = moveInput:z0() * frozenCoef --set this to zero if frozen to keep the ball locked
  if guardedWalkVector:length() > 1 then
    guardedWalkVector:normalize()
  end

  local desiredMovementVector = (worldSpaceMove and guardedWalkVector or cameraRotation * guardedWalkVector) * desiredMovementSpeed
  ...
  local maxBallAVAdjusted = linearScale(speedCoef, 0, 1, maxAllowedBallAVNormal, maxAllowedBallAVSprint)
...
local function updateGFX(dt)
  ...
  if playerInfo.firstPlayerSeated then
    obj:queueGameEngineLua(string.format(
      "if overhaul_walkEnterVehicle then overhaul_walkEnterVehicle.notePlayerMove(%s,%s) end",
      movementSpeedCoef, walkVector:length()
    ))
  end
end
...
local function setExternalMove(x, y, speedCoef)
  if x == nil then
    externalMove = nil
  else
    externalMove = {x = x or 0, y = y or 0, speedCoef = speedCoef}
  end
end

local function setExternalWorldMove(x, y, speedCoef)
  if x == nil then
    externalMove = nil
  else
    externalMove = {x = x or 0, y = y or 0, speedCoef = speedCoef, worldSpace = true}
  end
end
...
M.setExternalMove = setExternalMove
M.setExternalWorldMove = setExternalWorldMove
M.jump = jump
