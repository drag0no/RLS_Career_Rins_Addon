...
-- Runtime-only multipliers layered on top of typeMultipliers. Owned exclusively
-- by the activityHeat system (reward surges / saturation cool-downs). These are
-- NEVER persisted here and NEVER written by difficulty presets or challenges, so
-- the baseline economy balance is preserved whenever no surge/cool-down is active.
local transientMultipliers = {}
local TRANSIENT_FLOOR = 0.5
local TRANSIENT_CEIL = 5.0
local typeSources = {}
...
local function discoverActivityTypes()
    ...
    if gameplay_rlsTaxi then
        activityTypesFound["taxi"] = true
        typeSources["taxi"] = typeSources["taxi"] or {}
        typeSources["taxi"]["taxi_module"] = true
        
        if gameplay_rlsTaxi.getPassengerTypes then
            local passengerTypes = gameplay_rlsTaxi.getPassengerTypes()
    ...
    if gameplay_offroadRecovery then
        activityTypesFound["offroadRecovery"] = true
        typeSources["offroadRecovery"] = typeSources["offroadRecovery"] or {}
        typeSources["offroadRecovery"]["offroad_recovery_module"] = true
    end
    ...
    activityTypesFound["demo"] = true
    typeSources["demo"] = typeSources["demo"] or {}
    typeSources["demo"]["freeroam_module"] = true

    activityTypesFound["burnout"] = true
    typeSources["burnout"] = typeSources["burnout"] or {}
    typeSources["burnout"]["freeroam_module"] = true

    activityTypesFound["burnoutComp"] = true
    typeSources["burnoutComp"] = typeSources["burnoutComp"] or {}
    typeSources["burnoutComp"]["freeroam_module"] = true
...
local function saveMultipliers(currentSavePath)
    ...
    if not currentSavePath then
        _, currentSavePath = career_saveSystem.getCurrentProfile()
...
local function getTransientMultiplier(typeName)
    if not typeName then return 1.0 end
    return transientMultipliers[normalizeTypeName(typeName)] or 1.0
end

-- Set a transient (surge/cool-down) factor for a type. A value of 1.0 clears it.
-- Clamped to safety bounds; the activityHeat system applies its own tighter design clamps.
local function setTransientMultiplier(typeName, value)
    if not typeName then return false end
    typeName = normalizeTypeName(typeName)
    value = tonumber(value)
    if not value then return false end
    value = math.max(TRANSIENT_FLOOR, math.min(TRANSIENT_CEIL, value))
    if math.abs(value - 1.0) < 1e-6 then
        transientMultipliers[typeName] = nil
    else
        transientMultipliers[typeName] = value
    end
    return true
end

local function clearTransientMultipliers()
    transientMultipliers = {}
end

local function getTransientMultipliers()
    return deepcopy(transientMultipliers)
end
...
local function getSectionMultiplier(sectionName)
    ...
    if base == nil then
        base = explicitMultiplier or 1.0
    end
    
    -- Disabled stays disabled; the transient layer can never revive a 0 multiplier.
    if base == 0 then return 0 end
    return base * getTransientMultiplier(sectionName)
end
...
M.getTransientMultiplier = getTransientMultiplier
M.setTransientMultiplier = setTransientMultiplier
M.clearTransientMultipliers = clearTransientMultipliers
M.getTransientMultipliers = getTransientMultipliers
