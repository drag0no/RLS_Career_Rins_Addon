...
local function unstashVehicles()
  ...
  local startId = mission._startingInfo and mission._startingInfo.vehId
  if startId then
    local startObj = getObjectByID(startId)
    if startObj and not startObj:getActive() then
      startObj:setActive(1)
      return false
    end
  end
  ...
  -- Traffic/parking freeze already deleted these; nothing to restore per id.
  while setupData._unstashVehicleIds[1] do
    local peek = setupData._unstashVehicleIds[1]
    if setupData.stashedVehicles[peek] == true then break end
    table.remove(setupData._unstashVehicleIds, 1)
    setupData.stashedVehicles[peek] = nil
  end
...
local function taskStartVehicleStep(step)
  ...
  if not vehicleSetup.enabled then
    ...
    if (vehicleSetup.usePlayerVehicle or not vehicleSetup.vehicles or not idx) and not vehicleSetup.useCustomConfig then
      ...
      if veh then
        -- Despawn-mode stash deactivates the current career car even when this
        -- step later keeps it. Vehicle Lua is dead while inactive, so diffs
        -- and the part-condition snapshot ping never return (120s timeout).
        if not veh:getActive() then
          veh:setActive(1)
          return
        end
        if not veh:isReady() then
          return
        end
...
local function taskStartTakePartConditionSnapshot(step)
  ...
  -- Wait until vehicle Lua is alive. A one-shot ping to an inactive car never
  -- completes; vanilla then sits on the 120s step timeout.
  if not vehObj:getActive() then
    vehObj:setActive(1)
    return
  end
  if not vehObj:isReady() then
    return
  end
...
local function taskStopMissionPostStep(step)
  ...
  -- Career deleted traffic/parking for the mission. Don't rebuild the exact
  -- previous set (30+ parked cars) before fade; refill around the player.
  local trafficSetup = mission.setupModules.traffic
  if trafficSetup and not trafficSetup.usePrevTraffic then
    log("I", logTag, "Refilling traffic after mission (skip exact stash respawn)")
    if career_modules_playerDriving and career_modules_playerDriving.restoreTrafficAfterMission then
      career_modules_playerDriving.restoreTrafficAfterMission()
    elseif gameplay_traffic.getState() ~= "on" then
