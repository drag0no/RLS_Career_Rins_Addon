...
local function onBeamNGTrigger(data)
  ...
  if data.event == 'enter' and not gameplay_missions_missionManager.getForegroundMissionId() then
    if gameplay_police and gameplay_police.isVanillaTaxiExempt and gameplay_police.isVanillaTaxiExempt(data.subjectID) then
      return
    end
    if gameplay_taxi and gameplay_taxi.isTaxiRideActive and gameplay_taxi.isTaxiRideActive() and data.subjectID == be:getPlayerVehicleID(0) then
      return
    end
