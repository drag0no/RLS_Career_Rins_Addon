...
function C:init(id, role)
  ...
  self.id = id
  self.rlsTrafficVehicleOverrideVersion = 1
  self.state = 'reset'
...
function C:setRole(roleName) -- sets the driver role
  ...
  if self.isTaxi and gameplay_taxi and gameplay_taxi.getTaxiState then
    local taxiState = gameplay_taxi.getTaxiState(self.id)
    if taxiState and taxiState ~= "available" and taxiState ~= "unavailable" then
      return
    end
  end
...
function C:checkCollisions() -- checks for contact with other tracked vehicles
  local selfObj = getObjectByID(self.id)
  if not selfObj then
    return
  end
  ...
  for id, veh in pairs(map.objects) do
    if self.id ~= id then
      ...
      if collision then
        ...
        if isCurrentCollision then collision.damage = max(collision.damage, self.damage - self.prevDamage) end
        ...
        elseif isCurrentCollision and not collision.inArea then
          ...
          do
            local dvec = veh.pos - self.pos
            local dlen = dvec:length()
            collision.dot = (dlen > 1e-6) and self.driveVec:dot(dvec / dlen) or 0
          end
          self.collisionCount = self.collisionCount + 1

          if self.speed >= 1 then
            if self.role and self.role.onCollision then
              self.role:onCollision(id, collision)
            end

            for otherId, otherVeh in pairs(gameplay_traffic.getTrafficData()) do
              if not otherVeh.otherCollisionFlag and otherId ~= self.id and otherId ~= id and otherVeh.role and otherVeh.role.onOtherCollision then
        ...
        if self.isAi and not self.isTaxi then
          veh = gameplay_traffic.getTrafficData()[id]
          if veh and veh.isPerson and self.role and self.role.flags then
            if isCurrentCollision and not self.role.flags.pullOver then
              if self.role.setAction then self.role:setAction('pullOver') end
            elseif not isCurrentCollision and self.role.flags.pullOver and dist > square(collision.vehDist + 3) then
              if self.role.resetAction then self.role:resetAction() end
            end
          end
        end
      end
...
function C:triggerOffense(data) -- triggers a pursuit offense
  ...
  local scoreScale = (self.policeVars and self.policeVars.offenseScoreScale) or 0.75
  data.score = data.score * scoreScale
...
function C:checkOffenses() -- tests for vechicle offenses for police
  ...
  if gameplay_police and gameplay_police.isVanillaTaxiExempt and gameplay_police.isVanillaTaxiExempt(self.id) then
    return
  end
  local pursuit = self.pursuit
  local threshold = 1 - clamp(self.policeVars.strictness, 0, 0.8) -- offense threshold
  local speedLimit = self.tracking.speedLimit or 0
  local skipSpeedOffenses = not self.isAi and gameplay_police and gameplay_police.isPlayerSpeedingImmune and gameplay_police.isPlayerSpeedingImmune()
  local skipSpeedGrace = self.speedingGraceTimer and self.speedingGraceTimer > 0

  if not self.isPerson and not skipSpeedOffenses and not skipSpeedGrace and self.tracking.faults.overSpeed >= threshold then
    if self.speed >= max(16.7, speedLimit * 1.2) and not pursuit.offenses.speeding then -- at least 60 km/h
      self:triggerOffense({key = 'speeding', value = self.speed, threshold = speedLimit, score = 100})
    end
  end
  if not self.isPerson and not skipSpeedOffenses and not skipSpeedGrace and self.tracking.faults.overSpeed >= threshold then
    if self.speed >= max(27.8, speedLimit * 2) and not pursuit.offenses.racing then -- extra heat at 2x / 100 km/h
      self:triggerOffense({key = 'racing', value = self.speed, threshold = speedLimit, score = 200})
    end
  end
  ...
  for id, coll in pairs(self.collisions) do
    ...
    if veh then
      ...
      if not coll.offense and validCollision then
        if veh.role.name == 'police' and coll.inArea then
  ...
  if self.isAi then
    ...
    if self.respawnSpeed then
      ...
      if obj then
        ...
        obj:queueLuaCommand('thrusters.applyVelocity(obj:getDirectionVector() * '..self.respawnSpeed..', '..dt..')')
      end
...
C.__index = C
package.loaded['overhaul/trafficVehicleOverrideClass'] = C
