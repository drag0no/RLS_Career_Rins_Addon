...
local function buildGroupData()
  ...
  local groupData = {
    ...
    type_computer = {label = "bigMap.group.type.computer", icon = "laptop"},
    type_apm_computer = {label = "bigMap.group.type.computer", icon = "laptop"},
    type_events = {label = "Events", icon = "raceFlag"},
    type_roleplay = {label = "Roleplay", icon = "strobeLights"},
...
local function processNonMissionPoi(poi, groupData)
  ...
  elseif poi.data.type == "computer" then
    ...
    if poi.data.facility and poi.data.facility.functions and poi.data.facility.functions.apmLandingPage then
      filterData.groupTags['type_apm_computer'] = true
    end
  elseif poi.data.type == "events" then
    filterData.groupTags['type_events'] = true
  elseif poi.data.type == "busWork" or poi.data.type == "paramedicWork"
      or poi.data.type == "policeWork" or poi.data.type == "assignRole" then
    filterData.groupTags['type_roleplay'] = true
  ...
  if not career_career.isActive() then
    ...
    table.insert(groupStructure, {
      ...
      groups = {
        ...
        groupsById['type_events'],
        groupsById['type_roleplay'],
        groupsById['type_businesses'],
        groupsById['type_repairSpot'],
    ...
    table.insert(groupStructure, {
      ...
      groups = {
        ...
        groupsById['type_businesses'],
        groupsById['type_repairSpot'],
        groupsById['type_travel'],
        groupsById['type_miningHaul'],
        groupsById['type_facilityWork'],
        groupsById['type_other'],
      }
    })
...
local function setVisibleIds()
  ...
  for _, groupSection in ipairs(groupStructureCache) do
    for _, group in ipairs(groupSection.groups) do
      if filterVisibilityState[group.key] and not group.hideFromMap then
        for _, poiId in ipairs(group.elements) do
