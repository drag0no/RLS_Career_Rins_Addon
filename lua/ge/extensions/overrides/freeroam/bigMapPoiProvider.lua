...
local function initializeGroupVisibility()
  ...
  local defaultGroups = {
    ...
    'type_garage', 'type_gasStation', 'type_dealership', 'type_busWork', 'type_paramedicWork', 'type_policeWork', 'type_miningHaul', 'type_facilityWork', 'type_businesses', 'type_repairSpot', 'type_playerVehicle', 'type_other',
    'type_events', 'type_travel', 'type_assignRole',
  ...
  if cachedGroupData then
    for _, filter in ipairs(cachedFilters) do
      for _, group in ipairs(filter.groups) do
        if group and group.visible and not group.hideFromMap then
...
local poiTypeIcons = {
  ...
  busWork = 'bus',
  paramedicWork = 'strobeLights',
  policeWork = 'strobeLights',
  miningHaul = 'boxTruck',
  facilityWork = 'boxTruck',
  events = 'raceFlag',
  travel = 'fastTravel',
  assignRole = 'strobeLights',
  computer = 'laptop',
  tuningShop = 'wrench',
  racingTeam = 'raceFlag',
  repairSpot = 'wrench',
...
M.formatPoiForBigmap = function(poi)
  ...
  local icon = bmi.cardIcon or poiTypeIcons[poi.data.type] or "info"
  return {
    id = poi.id,
    icon = icon,
...
local function buildGroupData()
  ...
  local groupData = {
    ...
    type_busWork = {label = "Bus Work", icon = "bus"},
    type_paramedicWork = {label = "Paramedic Work", icon = "strobeLights"},
    type_policeWork = {label = "Police Work", icon = "strobeLights"},
    type_miningHaul = {label = "Mining Company", icon = "boxTruck"},
    type_facilityWork = {label = "Facility Work", icon = "boxTruck"},
    type_playerVehicle = {label = "Player Vehicles", icon = "carStarred"},
    type_businesses = {label = "Businesses", icon = "wrench"},
    type_repairSpot = {label = "Roadside Service", icon = "wrench"},
    -- Catchall for unknown / third-party POI types. List only; hide leftover map pins.
    type_other = {label = "Other", icon = "info", hideFromMap = true},
    type_events = {label = "Free-Roam Events", icon = "raceFlag"},
    type_travel = {label = "Travel Points", icon = "fastTravel"},
    type_assignRole = {label = "Role Assignment", icon = "strobeLights"},
...
local function processNonMissionPoi(poi, groupData, playerPos, distanceFilter)
  ...
  elseif poi.data.type == 'garage' or poi.data.type == 'computer' then
  ...
  elseif poi.data.type == "policeWork" then
    filterData.groupTags['type_policeWork'] = true
  elseif poi.data.type == "miningHaul" then
    filterData.groupTags['type_miningHaul'] = true
  elseif poi.data.type == "facilityWork" then
    filterData.groupTags['type_facilityWork'] = true
  elseif poi.data.type == "tuningShop" or poi.data.type == "racingTeam" then
    filterData.groupTags['type_businesses'] = true
  elseif poi.data.type == "repairSpot" then
    filterData.groupTags['type_repairSpot'] = true
  else -- uncategorized catchall (mods / future types with no dedicated group)
...
local function buildPremadeFilters(groupData)
  ...
  local filterCareerPois = {
    ...
    groups = {
      ...
      groupData['type_crawl'],
      groupData['type_events'],
      groupData['type_busWork'],
      groupData['type_paramedicWork'],
      groupData['type_policeWork'],
      groupData['type_miningHaul'],
      groupData['type_facilityWork'],
      groupData['type_businesses'],
      groupData['type_repairSpot'],
