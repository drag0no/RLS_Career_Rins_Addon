...
local function onExtensionLoaded()
    -- A Ctrl+L restore can load this module before gameplay_rlsTaxi is back, since
    -- the reload batch order is arbitrary. Skip registering rather than throwing:
    -- rlsTaxi.loadPassengerModules() unloads and reloads every passenger module when
    -- it initialises, which runs this again with the global in place.
    if not gameplay_rlsTaxi then return end
    gameplay_rlsTaxi.registerPassengerType("PARTY", {
        name = "Party Group",
        ...
        calculateTipBreakdown = function(fare, elapsedTime, speedFactor, passengerType)
            ...
            if gameplay_rlsTaxi.rideData and gameplay_rlsTaxi.rideData.currentSensorData then
                local sensorData = gameplay_rlsTaxi.rideData.currentSensorData
